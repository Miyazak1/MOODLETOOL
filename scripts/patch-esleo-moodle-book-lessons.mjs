...
function kindFromLabel(label) {
  ...
  if (lower.includes("expectation")) return "overview";
...
function lessonPlanFor(unit, lesson) {
  const path = `plans/source/UNIT ${unit}/ESLEO - Unit ${unit} - Lesson ${lesson} Lesson Plan.docx`;
...
function unitPlanFor(unit) {
  const path = `plans/source/UNIT ${unit}/ESLEO - Unit ${unit} - Unit Plan.docx`;
  if (!existsSync(join(courseRoot, path))) return null;
...
manifest.units = rawByUnit.map(({ unit, raw }) => {
  ...
  const lessons = raw.lessons.map((rawLesson, index) => {
    ...
    const rawPath = join(courseRoot, path, "book_pages_raw.json");
    mkdirSync(join(courseRoot, path), { recursive: true });
    writeJson(rawPath, rawPages);
  ...
  return {
    unit,
    title: previous.title && !/Lesson 1 Lesson Plan\.docx/i.test(previous.title)
      ? previous.title
      : `Unit ${unit}`,
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 75,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=75",
