...
manifest.units = rawByUnit.map(({ unit, raw }) => {
  ...
  const lessons = raw.lessons.map((rawLesson, index) => {
    ...
    const legacyId = paddedId(unit, lesson);
    ...
    const previousLesson =
      previousLessons.find((entry) => entry.id === id || entry.id === legacyId || Number(entry.lesson) === lesson) || {};
    const previousPlanPath = previousLesson.lessonPlan?.path;
    const lessonPlan = previousPlanPath && existsSync(join(courseRoot, previousPlanPath))
      ? fileRecord(`Lesson Plan - Unit ${unit} Lesson ${lesson}`, previousPlanPath, "lesson_plan", "lesson_plan")
      : null;
    return {
      id,
      unit,
      ...
      lessonPlan,
      ispring: (previousLesson.ispring || []).map(withoutIspringDownload),
  ...
  const previousUnitPlanPath = previous.unitPlan?.path;
  const unitPlan = previousUnitPlanPath && existsSync(join(courseRoot, previousUnitPlanPath))
    ? fileRecord(`Unit Plan - ${unitTitles[unit] || `Unit ${unit}`}`, previousUnitPlanPath, "unit_plan", "unit_plan")
    : null;
  return {
    unit,
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 87,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=87",
