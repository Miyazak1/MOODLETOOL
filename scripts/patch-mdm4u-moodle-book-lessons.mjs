...
const unitTitles = {
  1: "Statistics and One-Variable Data",
  2: "Probability and Counting",
  3: "Probability Distributions",
  4: "Data Collection and Analysis",
  5: "Culminating Data Management Project",
};
...
function lessonPlanFor(unit, lesson, previousLesson) {
  ...
  const candidates = [
    `plans/source/Unit ${unit}/Lesson Plan Unit ${unit} lesson ${lesson}.docx`,
    `plans/source/Unit ${unit}/Lesson Plan Unit ${unit} lesson ${lesson} .docx`,
  ];
...
manifest.units = rawByUnit.map(({ unit, raw }) => {
  ...
  const lessons = raw.lessons.map((rawLesson, index) => {
    ...
    return {
      ...
      lessonPlan,
      ispring: (previousLesson.ispring || []).map((item) => {
        const clean = { ...item };
        delete clean.downloadPath;
        delete clean.downloadUrl;
        delete clean.downloadBytes;
        return clean;
      }),
...
manifest.course = {
  ...(manifest.course || {}),
  code: course,
  title: "MDM4U · Mathematics of Data Management",
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 78,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=78",
