...
const courseRoot = join(workspaceRoot, "courseware", "ENG4U");
const manifestPath = join(courseRoot, "course-manifest.json");
const rawPath = join(courseRoot, "moodle-course-sections-raw", "answer-keys-7648.html");
const activityUrl = "https://www.esunnybrook.com/mod/assign/view.php?id=7648";
const activityRel = "localized-moodle-activities/assign/course-7648-2b6f04cd7d";
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "eng4u-answer-key-resource-patch/1.0");
...
function patchList(list) {
  for (const item of list || []) {
    if (item.moodleActivityId === "7648" || item.role === "answer_keys" || item.label === "Answer Keys") {
...
manifest.sourceAudit.eng4uAnswerKeysPatch = {
...
console.log(JSON.stringify(manifest.sourceAudit.eng4uAnswerKeysPatch, null, 2));
