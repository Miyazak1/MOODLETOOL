...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "eng2d-stmary-sections");
const BASE_URL = normalizeBaseUrl(process.env.STMARY_MOODLE_BASE_URL || "http://34.30.231.58");

const UNIT_TITLES = {
  1: "Unit 1: Macbeth",
  2: "Unit 2: Media and Advertising",
  3: "Unit 3: Language, Writing, and Literary Analysis",
  4: "Unit 4: Writing Skills and Literary Devices",
};
...
function filteredLinksForSection(sectionNo) {
  ...
  if (sectionNo === 6) return links.filter((link) => /\b(?:Exam|Final|Culminating)\b/i.test(link.text || ""));
  if (sectionNo === 7) return links.filter((link) => /\bAnswer\b/i.test(link.text || ""));
...
function isAssessmentActivity(item) {
  ...
  return /\b(AOL|Quiz|Test|Lab|Assignment|Exam|Final|Culminating)\b/i.test(label) && !/\bAnswer\b/i.test(label);
}
...
function attachHomeworkDownloadsToDropboxes(unitNo, unitResources, lessons) {
  for (const lesson of lessons || []) {
    ...
    if (homeworkSection) {
      const sectionExisting = new Set((homeworkSection.attachments || []).map((item) => item.path || item.downloadPath || item.label));
      const sectionAdditions = homeworkDownloads.filter((item) => !sectionExisting.has(item.path || item.downloadPath || item.label));
    ...
    const dropbox = (unitResources.lessonDropboxes || []).find((item) => {
      return new RegExp(`\\bUnit\\s+${unitNo}\\s+-\\s+Lesson\\s+${lesson.lesson}\\b`, "i").test(item.label || "");
    });
    if (!dropbox) continue;
    const existing = new Set((dropbox.attachments || []).map((item) => item.path || item.downloadPath || item.label));
    const additions = homeworkDownloads.filter((item) => !existing.has(item.path || item.downloadPath || item.label));
...
async function main() {
  ...
  for (const sectionNo of [1, 2, 3, 4, 5, 6, 7]) {
  ...
  finalSection.attachments = [
    ...(finalSection.attachments || []),
    ...[activityById.get("11467"), activityById.get("11468")].filter(hasMeaningfulResource),
  ];
  const courseSections = [courseStarter, courseOverview, finalSection, teacherPacket].filter(hasMeaningfulResource);
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "ENG2D · English, Grade 10, Academic",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 75",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
      ...
      downloadFailures,
      note: "New-site ENG2D crawl from Moodle course id 75. Moodle H5P Exit Card activities are excluded from the teacher course package unless local .h5p packages are separately available.",
    },
