...
function unitPlanDir(unit) {
  return `plans/source/BOH4M - Unit ${unit} - Lesson Plans & Unit Plans`;
}
...
function lessonPlanFor(unit, lesson, previousLesson) {
  ...
  const path = `${unitPlanDir(unit)}/Unit ${unit} - Lesson ${lesson} Lesson Plan.docx`;
...
function unitPlanFor(unit, previousUnit) {
  ...
  const path = `${unitPlanDir(unit)}/BOH4M - Unit ${unit} - Unit Plan.docx`;
  if (!existsSync(join(courseRoot, path))) return null;
...
function withoutIspringDownload(item) {
  if (!item) return item;
  const cleaned = { ...item };
  delete cleaned.downloadPath;
  delete cleaned.downloadUrl;
  delete cleaned.downloadBytes;
  return cleaned;
}
...
manifest.course = {
  ...(manifest.course || {}),
  code: course,
  title: "BOH4M · Business Leadership: Management Fundamentals",
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 71,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=71",
