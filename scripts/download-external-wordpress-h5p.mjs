...
if (!course) {
  console.error("Usage: node scripts/download-external-wordpress-h5p.mjs --course COURSE");
  process.exit(1);
}
    ...
    for (const id of ids) {
      ...
      try {
        ...
        const record = {
          ...
          role: String((rawPages.find((page) => String(page.html || "").includes(`id=${id}`))?.kind) || "").toLowerCase() === "consolidation" ? "consolidation" : "hands_on",
          path: relPath,
