...
const coursePage = "https://www.esunnybrook.com/course/view.php?id=40";
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-cia4u-manifest-builder/1.0");
...
function roleForLabel(label, mod = "") {
  ...
  if (/lesson plan/i.test(label)) return "lesson_plan";
  if (/chapter\s*\d+|demand and supply notes/i.test(label)) return "chapter_reading";
  if (/learning skills|work habits/i.test(label)) return "learning_skills";
  if (/attendance/i.test(label)) return "attendance_policy";
  if (/learning log/i.test(label)) return "learning_log";
  if (/observation.*check|conversation.*check/i.test(label)) return "observation_checklist";
  if (/discussion|forum/i.test(label) || mod === "forum") return "discussion";
  if (/test|exam/i.test(label)) return "assessment";
  if (/assignment|submission|a for l|a of l|afl|aal|aol/i.test(label)) return "assignment";
  if (/curriculum|ministry/i.test(label)) return "curriculum";
  if (/description|unit descriptions|strategies|assessment|evaluation|accommodation|types of assessment/i.test(label)) return "course_information";
...
const sectionSpecs = new Map([
  ["473", { unit: 0, title: "Introduction" }],
  ["474", { unit: 1, title: "Unit 1: Basic Economic Ideas" }],
  ["475", { unit: 2, title: "Unit 2: Productive Resources and Economic Systems" }],
  ["476", { unit: 3, title: "Unit 3: Demand and Supply" }],
  ["477", { unit: 4, title: "Unit 4: Demand and Supply: The Canadian Perspective" }],
  ["478", { unit: 5, title: "Unit 5: Application of Demand and Supply" }],
  ["479", { unit: 6, title: "Unit 6: Real Estate Market and Contract Law" }],
  ["480", { unit: 7, title: "Unit 7: Big and Small Businesses" }],
  ["481", { unit: 8, title: "Unit 8: Revision and Summative Assessment" }],
  ["482", { unit: 9, title: "Unit 9: Final Exam" }],
]);
...
const totalActivities = courseDownloads.length
  + units.reduce((sum, unit) => sum + unit.lessons.reduce((lessonSum, lesson) => lessonSum + (lesson.downloads?.length || 0), 0), 0);
...
const manifest = {
  ...
  course: {
    code: course,
    title: "Analysing Current Economic Issues, Grade 12, University",
...
console.log(`CIA4U manifest rebuilt: units ${units.length}; lessons ${manifest.sourceAudit.lessonCount}; activities ${totalActivities}.`);
