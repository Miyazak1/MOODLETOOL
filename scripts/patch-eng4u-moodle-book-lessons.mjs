...
function planFolder(unit) {
  return `plans/source/u${unit}/Lesson Plans _ Unit Plans`;
}

function lessonPlanFor(unit, lesson) {
  const path = `${planFolder(unit)}/Unit ${unit} - Lesson ${lesson} Lesson Plan.docx`;
...
function unitPlanFor(unit) {
  const path = `${planFolder(unit)}/ENG4U - Unit ${unit} - Unit Plan.docx`;
  if (!existsSync(join(courseRoot, path))) return null;
...
manifest.units = rawByUnit.map(({ unit, raw }) => {
  ...
  return {
    unit,
    title: previous.title && !/Unit Plan\.docx/i.test(previous.title) ? previous.title : `Unit ${unit}`,
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 72,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=72",
