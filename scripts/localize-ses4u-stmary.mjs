...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "ses4u-stmary-sections");
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-snc1d-stmary-localizer/1.0");
...
function cleanHtmlFragment(htmlValue, attachmentMap = new Map(), context = {}) {
  ...
  body = body.replace(/\s*src=["']https?:\/\/welcome\.hexstruct\.com\/wp-content\/plugins\/h5p\/h5p-php-library\/js\/h5p-resizer\.js["']\s+charset=["']UTF-8["']\s*&gt;/gi, "");
  body = body.replace(/\s*charset=["']UTF-8["']\s*&gt;/gi, "");
  body = body.replace(/<a\b[^>]*href=["'][^"']*files\/[^"']+["'][^>]*>\s*(?:\.|<br\s*\/?>)\s*<\/a>/gi, "");
  ...
  body = body.replace(/<iframe\b[^>]*src=["']([^"']+)["'][^>]*>\s*<\/iframe>/gi, (full, rawSrc) => {
    ...
    if (/hexstruct\.ispring\.com/i.test(src)) {
      ...
      if (local) return `<iframe class="localized-ispring" src="${escapeAttr(local)}" loading="lazy" allowfullscreen></iframe>`;
      return context.unit && context.lesson ? `<div class="portal-note">Interactive media pending local package; external playback was not embedded.</div>` : "";
    }
  ...
  body = body.replace(/\b(?:Completion requirements|Make a submission)\b/gi, "");
  return body.trim();
}

function localIspringHref(context = {}) {
  if (Number(context.section || 0) === 1 && context.pageRel) {
    const overviewRel = "ispring-localized/unit-00/course-overview/presentation.html";
    if (existsSync(join(COURSE_ROOT, overviewRel))) return relativeHref(context.pageRel, overviewRel);
  }
...
<head>
  ...
  <style>
    ...
    .localized-ispring, .ispring-player { border: 0; display: block; height: min(72vh, 760px); margin: 16px 0; width: 100%; }
    .embedded-h5p { margin: 16px 0 20px; width: 100%; }
...
function uniqueResources(items) {
  ...
  return items.filter((item) => {
    ...
    const normalizedSource = String(item.source || "").replace(/\?.*$/, "");
    const key = normalizedSource
      ? `${String(item.type || "").toLowerCase()}|${String(item.label || "").toLowerCase()}|${item.bytes || ""}|${normalizedSource.toLowerCase()}`
      : item.path;
...
async function localizeActivity(link) {
  ...
  if (!mod || !id || /Exit Card/i.test(link.text || "")) return null;
  const key = `${mod}:${id}`;
...
function filteredLinksForSection(sectionNo) {
  ...
  if (sectionNo === 7) return links.filter((link) => /Exam|Culminating/i.test(link.text || ""));
...
function categorizeUnitActivity(unitNo, items) {
  ...
  for (const item of items) {
    if (!item) continue;
    if (item.mod === "h5pactivity" || item.category === "moodle_h5pactivity") continue;
...
function curriculumGuideResources() {
  const rel = "texts/ontario-curriculum/2009science11_12.pdf";
  const abs = join(COURSE_ROOT, rel);
  if (!existsSync(abs)) return [];
  return [{
    ...
    textPreview: "Official Ontario Ministry curriculum guidance for Grades 11 and 12 Science, including SES4U Earth and Space Science, Grade 12, University Preparation.",
  }];
}
...
async function main() {
  ...
  const finalSection = await buildCourseSection(7, "Final Examination & Culminating", "course-sections/final-exam-culminating", "final_examination_culminating");
  ...
  for (const sectionNo of [1, 2, 3, 4, 5, 6, 7, 8]) {
  ...
  for (const unitNo of [1, 2, 3, 4, 5]) {
    ...
    const unitActivities = filteredLinksForSection(unitNo + 1)
      ...
      .filter(Boolean);
    const h5pActivitiesByLesson = new Map();
    for (const item of unitActivities.filter((activity) => activity.mod === "h5pactivity")) {
      const lessonNo = Number(/Lesson\s+(\d+)/i.exec(item.label || "")?.[1] || 0);
      if (!lessonNo) continue;
      if (!h5pActivitiesByLesson.has(lessonNo)) h5pActivitiesByLesson.set(lessonNo, []);
      h5pActivitiesByLesson.get(lessonNo).push({
        ...item,
        role: "lesson_h5p",
        teacherUse: "lesson_interactive",
      });
    }
    const lessons = book.lessons.map((lesson) => ({
      ...
      downloads: [...lesson.downloads, ...(h5pActivitiesByLesson.get(lesson.lessonNo) || [])],
      bookSections: lesson.bookSections,
      resourceCounts: {
        downloads: lesson.downloads.length + (h5pActivitiesByLesson.get(lesson.lessonNo) || []).length,
        bookSections: lesson.bookSections.length,
        ispring: iSpringForLesson(unitNo, lesson.lessonNo).length,
        h5p: (h5pActivitiesByLesson.get(lesson.lessonNo) || []).length,
      },
    }));
    units.push({
      unit: unitNo,
  ...
  const courseDownloads = [
    activityById.get("12280"),
    activityById.get("12281"),
    ...curriculumGuideResources(),
  ].filter(Boolean);
  const teacherResources = uniqueByPath([
    ...localizedActivities.filter((item) => /\bAnswer\b/i.test(item.label || "") && item.moodleActivityId !== "12419"),
    ...localizedActivities.filter((item) => item.moodleActivityId !== "12418" && /\b(AOL|Quiz|Test|Lab|Assignment|Exam|Culminating)\b/i.test(item.label || "")),
  ].filter(Boolean).map((item) => ({ ...item, teacherOnly: item.teacherOnly || /\bAnswer\b/i.test(item.label || "") || item.moodleActivityId === "12419" })));
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "SES4U · Earth and Space Science, Grade 12",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 84 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
      ...
      h5pActivityIncluded: units.reduce((sum, unit) => sum + unit.lessons.reduce((lessonSum, lesson) => lessonSum + (lesson.downloads || []).filter((item) => item.mod === "h5pactivity").length, 0), 0),
      h5pActivityExitCardsExcluded: [2, 3, 4, 5, 6].reduce((sum, sectionNo) => sum + sectionJson(sectionNo).modLinks.filter((link) => /Exit Card/i.test(link.text || "")).length, 0),
      textbookReference: {
        title: "",
        status: "not_checked",
        reason: "No SES4U textbook file has been confirmed yet; only Moodle course files and official public curriculum guidance are included.",
      },
      ...
      downloadFailures,
      note: "External H5P embeds are not displayed because local .h5p packages were not available. iSpring embeds are represented by local mirrored packages only.",
    },
    navigation: { primary: "unit", secondary: "lesson" },
    courseDownloads,
    texts: textResources(),
    units,
    courseSections: [courseOverview, finalSection],
    teacherResources,
    evaluations: localizedActivities.filter((item) => item.moodleActivityId !== "12418" && /\b(AOL|Quiz|Test|Lab|Assignment|Exam|Culminating)\b/i.test(item.label || "")),
  };
