...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "baf3m-stmary-sections");
const BASE_URL = normalizeBaseUrl(process.env.STMARY_MOODLE_BASE_URL || "http://34.30.231.58");

const UNIT_TITLES = {
  1: "Fundamental Accounting Practices",
  2: "Advanced Accounting Practices",
  3: "Internal Control, Budgeting, and Financial Analysis",
  4: "Accounting Ethics, Technology, and Careers",
};
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-baf3m-stmary-localizer/1.0");
...
async function main() {
  ...
  const courseDownloads = [
    activityById.get("10948"),
    activityById.get("10949"),
    activityById.get("11069"),
    activityById.get("11070"),
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "BAF3M · Financial Accounting Fundamentals, Grade 11",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 73 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
