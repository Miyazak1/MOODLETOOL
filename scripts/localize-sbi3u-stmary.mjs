...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "sbi3u-stmary-sections");
...
const pendingMedia = [];
const COURSE_INTRO_ACTIVITY_IDS = new Set(["6406", "6407"]);
...
function curriculumGuideResources() {
  ...
  if (!existsSync(abs)) return [];
  return [{
    label: "Ontario Curriculum, Grades 11 and 12: Science, 2008 (Revised)",
    type: "pdf",
    category: "curriculum_guidance",
    role: "curriculum_guideline",
    path: rel,
    previewPath: rel,
    downloadPath: rel,
    bytes: statSync(abs).size,
    source: "https://www.edu.gov.on.ca/eng/curriculum/secondary/2009science11_12.pdf",
    textPreview: "Official Ontario Ministry curriculum guidance for Grades 11 and 12 Science, including Biology, Grade 11, University Preparation (SBI3U).",
  }];
}
...
async function main() {
  ...
  const teacherPacket = await buildCourseSection(9, "Teacher Packet", "course-sections/teacher-packet", "teacher_packet");

  const localizedActivities = [];
  for (const sectionNo of [0, 1, 2, 3, 4, 5, 6, 7, 9]) {
  ...
  const courseDownloads = [
    activityById.get("6406") ? { ...activityById.get("6406"), role: "introduction" } : null,
    activityById.get("6407") ? { ...activityById.get("6407"), role: "introduction" } : null,
    activityById.get("6408"),
    activityById.get("6409"),
    activityById.get("6571"),
    activityById.get("6572"),
  ...
  const teacherResources = uniqueByPath([
    ...
    ...item,
    role: item.moodleActivityId === "6601" ? "answer_keys" : item.role,
    teacherOnly: item.teacherOnly || /\bAnswer\b/i.test(item.label || "") || item.moodleActivityId === "6601",
  })));
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "SBI3U · Biology, Grade 11",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 35 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
      ...
      textbookReference: {
        title: "Biology 11 textbook reference",
        status: "not_manifested",
        reason: "Only a Moodle/OSS source index was found; chapter PDFs returned AccessDenied and no legal local textbook PDF is available, so this is kept out of the user-facing texts section.",
      },
