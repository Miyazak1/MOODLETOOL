...
<body>
  <main>
    <article>
      ...
      <p>This local page records the unavailable Moodle URL activity so the course package does not keep a broken Moodle link as primary content.</p>
    </article>
  </main>
</body>
</html>
...
function eachResource(manifest, callback) {
  for (const item of manifest.courseDownloads || []) callback(item, null, "courseDownloads");
  for (const text of manifest.texts || []) {
    callback(text, null, "texts");
    for (const material of text.materials || []) callback(material, text, "textMaterial");
  }
  for (const unit of manifest.units || []) {
    callback(unit.unitPlan, unit, "unitPlan");
    for (const resource of Object.values(unit.unitResources || {})) {
      if (Array.isArray(resource)) {
        for (const item of resource) callback(item, unit, "unitResources");
      } else {
        callback(resource, unit, "unitResources");
      }
    }
    for (const lesson of unit.lessons || []) {
      callback(lesson.lessonPlan, lesson, "lessonPlan");
      for (const item of lesson.lessonText || []) callback(item, lesson, "lessonText");
      for (const item of lesson.textExports || []) callback(item, lesson, "textExports");
      for (const item of lesson.downloads || []) callback(item, lesson, "downloads");
      for (const item of lesson.ispring || []) callback(item, lesson, "ispring");
      for (const item of lesson.bookSections || []) callback(item, lesson, "bookSections");
    }
  }
}

function isThemeAttachment(attachment) {
  return /20260514205240_755_110\.png/i.test(attachment?.label || attachment?.path || "")
    || /\/theme_remui\/logo\//i.test(attachment?.source || "");
}
...
function removeThemeAttachments(manifest) {
  ...
  eachResource(manifest, (item) => {
    ...
    item.attachments = item.attachments.filter((attachment) => !isThemeAttachment(attachment));
...
function countThemeFilesOnDisk(root) {
  let count = 0;
  function visit(dir) {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      const path = join(dir, entry.name);
      if (entry.isDirectory()) visit(path);
      else if (entry.isFile() && /20260514205240_755_110\.png/i.test(entry.name)) count++;
    }
  }
...
function fillUnavailable(manifest) {
  const labels = new Map([
    ["Learning Skills and Work Habits", "Moodle URL activity returned HTTP 403 during localization."],
    ["Online Attendance Policy", "Moodle URL activity returned HTTP 403 during localization."],
  ]);
  const filled = [];
  eachResource(manifest, (item) => {
    if (!item || item.path || !labels.has(item.label)) return;
    const activityId = item.moodleActivityId || /id=(\d+)/.exec(item.url || "")?.[1] || item.label.toLowerCase().replace(/[^a-z0-9]+/g, "-");
    const rel = `localized-moodle-activities/unavailable/${activityId}-${item.label.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}/index.html`;
    const abs = join(courseRoot, rel);
    mkdirSync(dirname(abs), { recursive: true });
    writeFileSync(abs, unavailableHtml(item.label, labels.get(item.label)), "utf8");
    item.path = rel;
    item.type = "html";
    item.bytes = statSync(abs).size;
    item.source = item.url || item.source || "authenticated Moodle crawl";
    item.unavailable = true;
    item.unavailableReason = labels.get(item.label);
    delete item.url;
...
function writeSources(stats, removedThemeAttachments, unavailableItems) {
  ...
  const content = `# CGC1D Sources and Localization Notes

- Course source: authenticated SunnyBrook Moodle course shell, https://www.esunnybrook.com/course/view.php?id=43
- Structure: legacy Moodle activity course organized by the visible Moodle sections: Introduction, four course units, and Final Summative Project.
...
- Textbook/literary works: no course textbook, reader, or literary work index was exposed in the current Moodle shell; none was added.
- External URL activities: valid public/external targets are kept as local wrapper pages with external references. Moodle dashboard/javascript placeholders were removed.
...
function ensureSourceNotes(manifest) {
  ...
  manifest.courseDownloads.push({
    label: "CGC1D Sources and Localization Notes",
    type: "md",
...
function updateCatalog(stats) {
  ...
  if (entry) {
    entry.title = "Issues in Canadian Geography, Grade 9 Academic";
    entry.level = "Grade 9";
    entry.status = "ready";
    entry.manifestUrl = "/courseware/CGC1D/course-manifest.json";
    entry.baseUrl = "/courseware/CGC1D/";
    entry.notes = `Legacy Moodle activity package localized: ${stats.units} units, ${stats.lessons} activity groups; no textbook exposed in Moodle.`;
  }
...
function updateRoadmap(stats) {
  ...
  if (entry) {
    ...
    entry.moodle = {
      coursePage: "https://www.esunnybrook.com/course/view.php?id=43",
      outlineStatus: "ready",
    ...
    entry.localEvidence = {
      ...
      lessonPlans: 0,
      ispringFiles: 0,
      outlineExamples: ["localized-moodle-activities/resource/course-4393-9a9bd65dc0/9a9bd65dc0-CGC1D-Course-Outline.pdf"],
    };
    entry.nextActions = [
      "Review external URL wrappers during site QA; valid public targets are retained as external references.",
      "Add a textbook only if a confirmed current CGC1D Moodle/local source is provided.",
    ];
  }
...
const cleanupRemoved = Math.max(removedThemeAttachments, manifest.sourceAudit?.removedThemeAttachments || 0, countThemeFilesOnDisk(courseRoot));
const unavailableItems = unavailableLabels(manifest);
let stats = collectStats(manifest);
writeSources(stats, cleanupRemoved, unavailableItems);
...
manifest.sourceAudit = {
  ...
  unavailableResources: stats.unavailable,
  externalReferences: stats.externalReferences,
  removedThemeAttachments: cleanupRemoved,
  htmlFilesChanged,
...
console.log(JSON.stringify({ course, stats, removedThemeAttachments: cleanupRemoved, filledUnavailable, htmlFilesChanged, sourcesPath }, null, 2));
