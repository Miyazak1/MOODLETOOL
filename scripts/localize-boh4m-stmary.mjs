...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "boh4m-stmary-sections");
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-boh4m-stmary-localizer/1.0");
...
function filteredLinksForSection(sectionNo) {
  ...
  if (sectionNo === 7) return links.filter((link) => /^(Exam|Culminating)\s+Submission Dropbox$/i.test(link.text || ""));
  if (sectionNo === 8) return links.filter((link) => /Exit Card/i.test(link.text || ""));
...
function hasMeaningfulSection(item) {
  return Boolean(item?.attachments?.length || String(item?.textPreview || "").trim());
}
...
function courseOverviewIspringResource() {
  ...
  return {
    ...
    packagePath: posix.dirname(rel),
    bytes: statSync(abs).size,
    source: `${BASE_URL}/course/view.php?id=${COURSE_ID}&section=1`,
  };
}
...
async function main() {
  ...
  const finalSection = await buildCourseSection(7, "Final Examination & Culminating", "course-sections/final-examination-culminating", "culminating_assignment");
  const exitCardsSection = await buildCourseSection(8, "Exit Cards", "course-sections/exit-cards", "exit_cards");
  ...
  for (const unitNo of [1, 2, 3, 4, 5]) {
    ...
    const unitActivityLinks = [
      ...filteredLinksForSection(unitNo + 1),
      ...filteredLinksForSection(8).filter((link) => new RegExp(`\\bUnit\\s+${unitNo}\\b`, "i").test(link.text || "")),
    ];
    const unitActivities = unitActivityLinks
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "BOH4M · Business Leadership: Management Fundamentals, Grade 12, University",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 40",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
      ...
      h5pActivityExitCardsLocalized: localizedActivities.filter((item) => /Exit Card/i.test(item.label || "")).length,
    ...
    texts: textResources(),
    units,
    courseSections: [courseStarter, courseOverview, finalSection, exitCardsSection].filter((item) => hasMeaningfulSection(item) && !isEmptyTeacherPacketShell(item)),
    teacherResources,
