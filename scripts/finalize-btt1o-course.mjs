...
function collectStats(manifest) {
  ...
  return {
    ...
    pdf: byType(/pdf/i),
    doc: byType(/docx?$/i),
    spreadsheet: byType(/xlsx?|spreadsheet/i),
...
function writeSources(stats, removedThemeAttachments) {
  ...
  const content = `# BTT1O Sources and Localization Notes

- Course source: authenticated SunnyBrook Moodle course shell, https://www.esunnybrook.com/course/view.php?id=31
- Structure: legacy Moodle activity course organized by the visible Moodle sections: website creation, Microsoft Word activities, Excel activities, PowerPoint/civics-style presentation activities, and culminating final.
- Localized structure: ${stats.units} units, ${stats.lessons} lesson/activity groups, ${stats.resources} local resource records.
- Course documents and activities: Moodle announcements, assignment pages, rubrics, PDF/DOC/DOCX attachments, Excel workbook, XLS budget file, answer key, PowerPoint assignment materials, and culminating final files were localized where available.
- Course outline: no BTT1O course outline resource was exposed in the current Moodle shell or found in local courseware during this pass; none was added.
...
function ensureSourceNotes(manifest) {
  ...
  manifest.courseDownloads.push({
    label: "BTT1O Sources and Localization Notes",
    type: "md",
...
function updateCatalog(stats) {
  ...
  if (entry) {
    entry.title = "Information and Communication Technology in Business, Grade 9, Open";
    entry.level = "Grade 9";
    entry.status = "ready";
    entry.manifestUrl = "/courseware/BTT1O/course-manifest.json";
    entry.baseUrl = "/courseware/BTT1O/";
    entry.notes = `Legacy Moodle activity package localized: ${stats.units} units, ${stats.lessons} activity groups; course outline not exposed in current Moodle shell.`;
  }
...
function updateRoadmap(stats) {
  ...
  if (entry) {
    ...
    entry.moodle = {
      coursePage: "https://www.esunnybrook.com/course/view.php?id=31",
      outlineStatus: "not-exposed",
      outlineUrl: "",
      bookCount: 0,
    ...
    entry.nextActions = [
      "Add the course outline only if a confirmed current BTT1O Moodle/local source is provided.",
      "Review spreadsheet files in site QA for download behavior.",
    ];
  }
