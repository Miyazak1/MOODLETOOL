...
<body>
  <main>
    <article>
      ...
      <p>This local page records the unavailable Moodle activity so the course package does not keep a broken Moodle link as primary content.</p>
    </article>
  </main>
</body>
</html>
...
function fillUnavailable(manifest) {
  const reasons = new Map([
    ["COURSE CALENDAR", "The current Moodle resource activity returned HTTP 404 during localization."],
    ["MLA Citation Guide", "The current Moodle resource activity returned HTTP 404 during localization."],
  ]);
  const filled = [];
  eachResource(manifest, (item) => {
...
function updateUnitSummaries(manifest) {
  for (const unit of manifest.units || []) {
    ...
    for (const lesson of unit.lessons || []) {
      ...
      for (const item of lesson.bookSections || []) add(item);
...
function writeSources(stats, removedThemeAttachments, unavailableItems) {
  ...
  const content = `# CHC2D Sources and Localization Notes

- Course source: authenticated SunnyBrook Moodle course shell, https://www.esunnybrook.com/course/view.php?id=42
- Structure: legacy Moodle activity course organized by the visible Moodle sections: Introduction, four course units, Final Evaluation, and Teacher Evaluation Comments.
- Localized structure: ${stats.units} units, ${stats.lessons} lesson/activity groups, ${stats.resources} local resource records.
- Course documents: the Moodle course outline and CHC2D Unit Plans folder were localized from the current Moodle shell.
...
- Lesson plans: no separate lesson plan files were added unless exposed by the current Moodle shell.
`;
...
function ensureSourceNotes(manifest) {
  ...
  manifest.courseDownloads.push({
    label: "CHC2D Sources and Localization Notes",
    type: "md",
...
function updateCatalog(stats) {
  ...
  if (entry) {
    entry.title = "Canadian History since World War I, Grade 10, Academic";
    entry.level = "Grade 10";
    entry.status = "ready";
    entry.manifestUrl = "/courseware/CHC2D/course-manifest.json";
    entry.baseUrl = "/courseware/CHC2D/";
...
function updateRoadmap(stats) {
  ...
  if (entry) {
    ...
    entry.moodle = {
      coursePage: "https://www.esunnybrook.com/course/view.php?id=42",
      outlineStatus: "ready",
    ...
    entry.localEvidence = {
      ...
      lessonPlans: 0,
      ispringFiles: 0,
      outlineExamples: ["localized-moodle-activities/resource/course-4257-a96b8dc1d5/a96b8dc1d5-CHC2D-Course-Outline.pdf"],
    };
    entry.nextActions = [
      "Review external URL wrappers during site QA; valid public targets are retained as external references.",
      "Add a textbook only if a confirmed current CHC2D Moodle/local source is provided.",
    ];
  }
