...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "snc2d-stmary-sections");
...
const pendingMedia = [];
const COURSE_INTRO_ACTIVITY_IDS = new Set(["9089"]);
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-sbi3u-stmary-localizer/1.0");
...
async function localizeActivity(link) {
  ...
  try {
    ...
    if (/\bAnswer\b/i.test(title) && !isNumberedHomeworkAnswerTitle(title)) item.teacherOnly = true;
    activityCache.set(key, item);
    return item;
  } catch (error) {
...
function inferActivityRole(title, mod) {
  ...
  if (isNumberedHomeworkAnswerTitle(title)) return "homework_answer_page";
  if (isNumberedHomeworkSubmissionTitle(title)) return "homework_submission_page";
...
function inferTeacherUse(title) {
  if (isNumberedHomeworkAnswerTitle(title)) return "homework_answer_reference";
  if (isNumberedHomeworkSubmissionTitle(title)) return "student_submission";
...
function isNumberedHomeworkSubmissionTitle(title) {
  return /^Unit\s+\d+\s+-\s+Lesson\s+\d+\s*$/i.test(String(title || "").trim());
}

function isNumberedHomeworkAnswerTitle(title) {
  return /^Unit\s+\d+\s+-\s+Lesson\s+\d+\s*\(Answer\)\s*$/i.test(String(title || "").trim());
}

function asHomeworkSubmissionFolderItem(item) {
  if (!item) return null;
  if (isNumberedHomeworkAnswerTitle(item.label)) {
    return {
      ...item,
      role: "homework_answer_page",
      teacherUse: "homework_answer_reference",
      parentSection: "Homework Submission Folder",
      sourceGroup: "homework_submission_folder",
    };
  }
  if (isNumberedHomeworkSubmissionTitle(item.label)) {
    return {
      ...item,
      role: "homework_submission_page",
      teacherUse: "student_submission",
      parentSection: "Homework Submission Folder",
      sourceGroup: "homework_submission_folder",
    };
  }
...
function homeworkLabelKey(label) {
  const match = /^Unit\s+(\d+)\s+-\s+Lesson\s+(\d+)(?:\s*\((Answer)\))?\s*$/i.exec(String(label || "").trim());
  if (!match) return { unit: 999, lesson: 999, answer: 0 };
  return { unit: Number(match[1]), lesson: Number(match[2]), answer: match[3] ? 1 : 0 };
}

function sortHomeworkSubmissionItems(items) {
  return [...items].sort((a, b) => {
    const left = homeworkLabelKey(a.label);
    const right = homeworkLabelKey(b.label);
...
function attachBookHomeworkFilesToSubmissionItems(items, units) {
  const homeworkByKey = new Map();
  for (const unit of units) {
    for (const lesson of unit.lessons || []) {
      const homeworkSection = (lesson.bookSections || []).find((section) => String(section.sectionLabel || "").toLowerCase() === "homework");
      if (homeworkSection?.attachments?.length) homeworkByKey.set(`${unit.unit}:${lesson.lesson}`, homeworkSection.attachments.map(maybePreviewPath));
    }
  }
  return items.map((item) => {
    if (!isNumberedHomeworkSubmissionTitle(item.label)) return item;
    const key = homeworkLabelKey(item.label);
    const additions = homeworkByKey.get(`${key.unit}:${key.lesson}`) || [];
    if (!additions.length) return item;
    const seen = new Set((item.attachments || []).map((attachment) => attachment.path || attachment.downloadPath || attachment.label));
    const attachments = [...(item.attachments || [])];
    for (const attachment of additions) {
      const attachmentKey = attachment.path || attachment.downloadPath || attachment.label;
      if (!seen.has(attachmentKey)) attachments.push(attachment);
    }
...
function filteredLinksForSection(sectionNo) {
  ...
  if (sectionNo === 6) return links.filter((link) => /Final|Exam|Culminating/i.test(link.text || "") && !/\bUnit\s+\d+\b/i.test(link.text || ""));
  if (sectionNo === 7) return links.filter((link) => /Answer Keys/i.test(link.text || ""));
...
function categorizeUnitActivity(unitNo, items) {
  const buckets = { evaluations: [], reflectionAndLogs: [], discussions: [] };
  for (const item of items) {
    if (!item) continue;
    if (isNumberedHomeworkSubmissionTitle(item.label) || isNumberedHomeworkAnswerTitle(item.label)) continue;
    if (/KWL|Reflection|Learning log/i.test(item.label || "")) buckets.reflectionAndLogs.push(item);
...
function textResources() {
  const resources = ensureTextSupportResources();
  return [
    {
      id: "ontario-science-curriculum-9-10",
      title: "SNC2D · Science, Grade 10, Academic · Ontario Curriculum, Grades 9 and 10: Science, 2008 (Revised)",
      ...
      notes: "Official Ontario curriculum reference containing SNC2D Science, Grade 10, Academic expectations.",
      materials: [resources.curriculum],
      path: resources.curriculum.path,
      bytes: resources.curriculum.bytes,
      category: resources.curriculum.category,
      role: resources.curriculum.role,
      previewPath: resources.curriculum.previewPath,
      downloadPath: resources.curriculum.downloadPath,
    },
    {
      id: "snc2d-source-audit",
      title: "SNC2D Text And Source Audit",
      type: "source_audit",
      units: [1, 2, 3, 4],
      copyrightStatus: "local_audit_note",
      sourceStatus: "created_from_local_source_review",
      notes: "Records the St. Mary source, ON Science 10 textbook reference, official curriculum inclusion, and exclusion of unmatched local science textbooks.",
      materials: [resources.sourceAudit],
      path: resources.sourceAudit.path,
      bytes: resources.sourceAudit.bytes,
      category: resources.sourceAudit.category,
      role: resources.sourceAudit.role,
      downloadPath: resources.sourceAudit.downloadPath,
    },
  ];
}

function curriculumGuideResources() {
  const rel = "texts/ontario-curriculum/science910_2008.pdf";
  const abs = join(COURSE_ROOT, rel);
  if (!existsSync(abs)) return [];
  return [{
    label: "SNC2D · Science, Grade 10, Academic · Ontario Curriculum, Grades 9 and 10: Science, 2008 (Revised)",
    type: "pdf",
    ...
    downloadPath: rel,
    bytes: statSync(abs).size,
    source: "https://www.edu.gov.on.ca/eng/curriculum/secondary/science910_2008.pdf",
    textPreview: "Official Ontario Ministry curriculum guidance for Grades 9 and 10 Science, including Science, Grade 10, Academic (SNC2D).",
    parentSection: "Course Overview",
    sourceGroup: "official_curriculum",
    publisher: "Ontario Ministry of Education",
    copyrightStatus: "official_public_document",
    sourceStatus: "localized_from_public_official_source",
  }];
...
function ensureTextSupportResources() {
  ensureDir(join(COURSE_ROOT, "texts"));
  const textbookReferenceRel = "texts/on-science-10-textbook-reference/index.html";
  const textbookReferenceHtml = `
    <section>
      <p><strong>Course:</strong> SNC2D · Science, Grade 10, Academic</p>
      <p><strong>Referenced textbook:</strong> ON Science 10</p>
      <p><strong>ISBN:</strong> 9780070722224</p>
      <p><strong>Publisher:</strong> McGraw-Hill Ryerson / Nelson</p>
      <p>The local textbook list identifies <strong>SNC2D</strong> with <strong>ON Science 10</strong>. Public bibliographic records identify the same ISBN as <em>ON Science 10</em>. A legal full-text local PDF was not found in <code>D:\\工作文件\\SUNNYBROOK\\docs</code>, so this course includes the verified reference only.</p>
    </section>
  `;
  const textbookReferencePage = renderPage("SNC2D · Science, Grade 10, Academic · ON Science 10 Textbook Reference", textbookReferenceHtml, [], textbookReferenceRel);
  ensureDir(dirname(join(COURSE_ROOT, textbookReferenceRel)));
  writeFileSync(join(COURSE_ROOT, textbookReferenceRel), textbookReferencePage, "utf8");

  const sourceAuditRel = "texts/SOURCES.md";
  const sourceAuditMd = `# SNC2D Text And Source Audit

Updated: ${new Date().toISOString()}

- Primary course content source: authenticated St. Mary Moodle course shell at ${BASE_URL}/course/view.php?id=${COURSE_ID}.
- Structure: St. Mary Moodle course id ${COURSE_ID} with Course Overview, four Moodle lesson books, course-level documents, evaluations, Homework Submission Folder pages, Teacher Packet / Answer Keys, local iSpring, and local H5P previews.
- Referenced textbook: ON Science 10.
- ISBN: 9780070722224.
- Evidence: local file \`docs/Textbook-list-June-2022-2023.pdf\` lists \`SNC2D On Science 10 9780070722224\`; public bibliographic records identify ISBN 9780070722224 as \`ON Science 10\`.
- Full textbook status: no legal local ON Science 10 textbook PDF was found in \`D:\\工作文件\\SUNNYBROOK\\docs\`, so no commercial textbook full text is included.
- Official curriculum: The Ontario Curriculum, Grades 9 and 10: Science, 2008 (Revised). Local file: \`texts/ontario-curriculum/science910_2008.pdf\`. Source: https://www.edu.gov.on.ca/eng/curriculum/secondary/science910_2008.pdf.
- Excluded local docs: Biology 11, Biology 12, Chemistry 11/12, Physics 11/12, and unrelated Nelson/McGraw-Hill files in \`docs\` are different courses/grades and were not substituted for SNC2D.
- iSpring/video/H5P policy: local playback/share only; iSpring/video downloads are not exposed.
`;
  writeFileSync(join(COURSE_ROOT, sourceAuditRel), sourceAuditMd, "utf8");

  const curriculum = curriculumGuideResources()[0] || null;
  const textbookReference = {
    label: "SNC2D · Science, Grade 10, Academic · ON Science 10 Textbook Reference",
    type: "html",
    category: "textbook_reference",
    role: "textbook_reference",
    path: textbookReferenceRel,
    previewPath: textbookReferenceRel,
    bytes: statSync(join(COURSE_ROOT, textbookReferenceRel)).size,
    source: "docs/Textbook-list-June-2022-2023.pdf and public bibliographic records for ISBN 9780070722224",
    textPreview: "Verified textbook reference for SNC2D: ON Science 10, ISBN 9780070722224. Full textbook PDF is not included because no legal local copy was found.",
    parentSection: "Course Overview",
    sourceGroup: "textbook_reference",
  };
  const sourceAudit = {
    label: "SNC2D Text And Source Audit",
    type: "md",
    category: "source_audit",
    role: "source_audit",
    path: sourceAuditRel,
    downloadPath: sourceAuditRel,
    bytes: statSync(join(COURSE_ROOT, sourceAuditRel)).size,
    source: "local source audit",
    textPreview: "Records St. Mary source, ON Science 10 textbook reference, official curriculum inclusion, and exclusions.",
    parentSection: "Course Overview",
    sourceGroup: "source_audit",
  };
  return { textbookReference, curriculum, sourceAudit };
}
...
async function main() {
  ...
  const textMaterials = textResources();
  const courseTextDownloads = textMaterials.flatMap((text) => text.materials || []).filter(Boolean);

  const homeworkSubmissionItems = attachBookHomeworkFilesToSubmissionItems(
    sortHomeworkSubmissionItems(
      uniqueByPath(
        [2, 3, 4, 5]
          .flatMap((sectionNo) => filteredLinksForSection(sectionNo))
          .filter((link) => !/\/mod\/book\//i.test(link.href))
          .map((link) => activityById.get(parseActivityUrl(link.href).id))
          .map(asHomeworkSubmissionFolderItem)
          .filter(Boolean)
  ...
  const courseDownloads = [
    activityById.get("9089") ? { ...activityById.get("9089"), role: "introduction" } : null,
    activityById.get("9090"),
    activityById.get("9091"),
    activityById.get("9278"),
    ...homeworkSubmissionItems,
    ...courseTextDownloads,
  ].filter(Boolean);
  const introductionResources = courseDownloads.filter((item) => item.role === "introduction");
  const teacherResources = uniqueByPath([
    activityById.get("11072"),
    ...localizedActivities.filter((item) => /^Answer Keys$/i.test(item.label || "")),
    ...localizedActivities.filter(isAssessmentActivity),
  ].filter(Boolean).filter(hasDisplayablePayload).map((item) => ({
    ...item,
    role: item.moodleActivityId === "11072" ? "answer_keys" : item.role,
    teacherOnly: item.teacherOnly || /^Answer Keys$/i.test(item.label || "") || item.moodleActivityId === "11072",
  })));
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "SNC2D · Science, Grade 10, Academic",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 57 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
      ...
      h5pActivityExitCardsExcluded: sectionJson(8).modLinks.filter((link) => /h5pactivity|Exit Card/i.test(`${link.href} ${link.text}`)).length,
      textbookReference: {
        title: "ON Science 10",
        isbn13: "9780070722224",
        publisher: "McGraw-Hill Ryerson / Nelson",
        status: "referenced_only",
        evidence: [
          "docs/Textbook-list-June-2022-2023.pdf lists: SNC2D On Science 10 9780070722224",
          "Public bibliographic/product records identify ISBN 9780070722224 as ON Science 10",
        ],
        reason: "No confirmed legal local ON Science 10 full-text PDF was identified in docs, so only the textbook reference page is included.",
      },
      curriculumGuidance: courseTextDownloads.filter((item) => item.role === "curriculum_reference").map((item) => ({
        title: item.label,
        status: "manifested",
        source: item.source,
        path: item.path,
      ...
      homeworkSubmissionFolder: {
        items: homeworkSubmissionItems.length,
        note: "St. Mary numbered Unit X - Lesson Y and Unit X - Lesson Y (Answer) activities are preserved in Moodle source order under Homework Submission Folder, matching MDM4U.",
      },
      note: "This rebuild uses St. Mary Moodle as the source of truth. External H5P embeds are marked pending by the initial Moodle pass; run localize-stmary-wordpress-h5p-embeds.mjs to replace them with local H5P packages. iSpring embeds are represented by local mirrored packages only.",
    },
    navigation: { primary: "unit", secondary: "lesson" },
    courseDownloads,
    texts: textMaterials,
    units,
    courseSections: [courseStarter, ...introductionResources, courseOverview, finalSection].filter((item) => !isEmptyTeacherPacketShell(item)),
    teacherResources,
