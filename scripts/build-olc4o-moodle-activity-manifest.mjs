...
const coursePage = "https://www.esunnybrook.com/course/view.php?id=9";
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-olc4o-manifest-builder/1.0");
...
function roleForLabel(label, mod = "") {
  ...
  if (/lesson plans?/i.test(label) || mod === "folder") return "lesson_plan_folder";
  if (/course planning|learning goals|success criteria/i.test(label)) return "course_planning";
  if (/rubric|checklist|organizer|mla|citation|sample/i.test(label)) return "reference_resource";
  if (/grammar|resource/i.test(label)) return "lesson_material";
  if (/lesson\s*\d+|speech:|article|poem|short story|background/i.test(label)) return "lesson_material";
  if (/discussion|forum/i.test(label) || mod === "forum") return "discussion";
  if (/activity|assignment|submission|dropbox|portfolio|brochure|vocabulary log|tracking sheet|reading log/i.test(label)) return "assignment";
  if (/final exam|exam/i.test(label)) return "assessment";
...
const sectionSpecs = new Map([
  ["40", { unit: 0, title: "Introduction" }],
  ["41", { unit: 0, title: "OLC4O Course Documents" }],
  ["42", { unit: 0, title: "Resources" }],
  ["43", { unit: 1, title: "Unit 1: Reading and Writing for Personal Success" }],
  ["44", { unit: 2, title: "Unit 2: Community Voices Through Reading and Writing" }],
  ["45", { unit: 3, title: "Unit 3: Reading and Writing as Community Action" }],
  ["47", { unit: 4, title: "Unit 4: Culminating Project - Demonstrating Success in Reading and Writing" }],
  ["48", { unit: 5, title: "Unit 5: Final Exam" }],
]);
...
const manifest = {
  ...
  course: {
    code: course,
    title: "Ontario Secondary School Literacy Course, Grade 12, Open",
...
console.log(`OLC4O manifest rebuilt: units ${units.length}; lessons ${manifest.sourceAudit.lessonCount}; activities ${totalActivities}.`);
