...
const course = "EPS3O";
const title = "Presentation and Speaking Skills, Grade 11, Open";
...
function scrubManifestSources(manifest) {
  ...
  if (/www\.esunnybrook\.com/i.test(manifest.sourceAudit?.coursePage || "")) {
    manifest.sourceAudit.coursePage = "Moodle course id 10";
    scrubbed++;
  }
  eachResource(manifest, (item) => {
    if (!item) return;
...
function collectStats(manifest) {
  ...
  return {
    ...
    pdf: byType(/pdf/i),
    doc: byType(/docx?$/i),
    lessonPlans: resources.filter((item) => /lesson[- ]?plan/i.test(`${item.label || ""} ${item.path || ""}`) && /docx?/i.test(`${item.type || ""} ${item.path || ""}`)).length,
...
function writeSources(stats, folderPagesRewritten, removedTransientAttachments, youtubePages) {
  ...
  const content = `# EPS3O Sources and Localization Notes

- Course source: authenticated SunnyBrook Moodle course shell, course id 10.
- Structure: legacy Moodle activity course organized by the visible Moodle sections: Introduction, Unit 1 Understanding Presentations - Speaking for Yourself, Unit 2 Understanding Presentations - Speaking for the Classroom, Unit 3 Making Presentations - Speaking for the Workplace, Unit 4 Making Presentations - Speaking for the Community, and Final Evaluation.
...
- Course documents: EPS3O course outline PDF was localized from Moodle.
- Lesson plans: ${stats.lessonPlans} Moodle-exposed unit lesson-plan DOCX file(s) were localized from the current course shell.
...
- Teacher resources: teacher observation/conversation checklists and final learning-skills evaluation PDF were retained because they are blank/current course assessment resources, not named student feedback.
...
function ensureSources(manifest) {
  ...
  manifest.courseDownloads.push({
    label: "EPS3O Sources and Localization Notes",
    type: "md",
...
function updateCatalog(stats) {
  ...
  if (entry) {
    ...
    entry.status = "ready";
    entry.manifestUrl = "/courseware/EPS3O/course-manifest.json";
    entry.baseUrl = "/courseware/EPS3O/";
...
function updateRoadmap(stats) {
  ...
  if (entry) {
    ...
    entry.moodle = { coursePage: "Moodle course id 10", outlineStatus: "ready", outlineUrl: "", bookCount: 0, numberedLessonCount: stats.lessons };
    entry.readiness = {
      units: stats.units,
    ...
    entry.localEvidence = { courseOutlines: 1, unitPlans: 0, lessonPlans: stats.lessonPlans, ispringFiles: 0, outlineExamples: ["EPS3O Course Outline PDF", "Unit Lesson Plan DOCX folders"] };
    entry.nextActions = [];
  }
...
manifest.sourceAudit = {
  ...manifest.sourceAudit,
  coursePage: "Moodle course id 10",
  ...
  lessonPlanStatus: `${stats.lessonPlans} Moodle lesson-plan DOCX file(s) localized from lesson-plan folders`,
  teacherResourceStatus: "blank teacher checklists and learning-skills evaluation retained; no named student feedback found",
...
updateCatalog(stats);
updateRoadmap(stats);
console.log(`EPS3O finalized: units ${stats.units}; lessons ${stats.lessons}; resources ${stats.resources}; attachments ${stats.attachments}; lesson plans ${stats.lessonPlans}; folders ${folderPagesRewritten}.`);
