...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "chc2d-stmary-sections");
const BASE_URL = normalizeBaseUrl(process.env.STMARY_MOODLE_BASE_URL || "http://34.30.231.58");

const UNIT_TITLES = {
  1: "Canada in the First World War and the Roaring Twenties",
  2: "The Great Depression and the Second World War",
  3: "Canada from 1945 to 1982",
  4: "Canada from 1982 to Present",
  5: "Culminating & Final Summative",
};
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-chc2d-stmary-localizer/1.0");
...
function filteredLinksForSection(sectionNo) {
  ...
  if (sectionNo === 6) return links.filter((link) => /Culminating|Final Summative/i.test(link.text || ""));
...
async function main() {
  ...
  const finalSection = await buildCourseSection(6, "Culminating & Final Summative", "course-sections/culminating-final-summative", "culminating_assignment");
  ...
  units.push({
    ...
    lessons: [{
      ...
      unit: 5,
      lesson: 1,
      title: "Culminating & Final Summative",
      path: "course-sections/culminating-final-summative",
      bookPageCount: 1,
      lessonText: [],
      textExports: [],
      lessonPlan: null,
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "CHC2D · Canadian History since World War I, Grade 10, Academic",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 83 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
    ...
    texts: textResources(),
    units,
    courseSections: [courseStarter, courseOverview, finalSection].filter((item) => hasMeaningfulSection(item) && !isEmptyTeacherPacketShell(item)),
    teacherResources,
