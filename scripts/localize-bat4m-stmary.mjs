...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "bat4m-stmary-sections");
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-bat4m-stmary-localizer/1.0");
...
async function main() {
  ...
  const courseDownloads = [
    activityById.get("10811"),
    activityById.get("10812"),
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "BAT4M · Financial Accounting Principles, Grade 12, University",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 72 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
