  ...
  while (stack.length) {
    ...
    for (const entry of readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        const rel = relative(courseRoot, absolute).replaceAll("\\", "/");
        if (!rel.includes("/OLD/")) files.push(rel);
      }
    }
  }
  return files;
...
function findPlanPath(pattern) {
  return planFiles.find((file) => pattern.test(file)) || null;
}

function lessonPlanFor(unit, lesson) {
  const path = findPlanPath(new RegExp(`^plans/source/Plans/SCH3U - Unit ${unit} - Unit & Lesson Plans/SCH3U-\\s*Unit\\s*${unit}\\s*-\\s*Lesson\\s*${lesson}\\.docx$`, "i"));
  if (!path) return null;
  return planningRecord(`Lesson Plan - Unit ${unit} Lesson ${lesson}`, path, "lesson_plan", "lesson_plan");
}

function unitPlanFor(unit) {
  const path = findPlanPath(new RegExp(`^plans/source/Plans/SCH3U - Unit ${unit} - Unit & Lesson Plans/SCH3U - Unit ${unit} - Unit Plan\\.docx$`, "i"));
  if (!path) return null;
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 81,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=81",
