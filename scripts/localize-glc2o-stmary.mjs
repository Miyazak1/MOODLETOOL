...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "glc2o-stmary-sections");
const BASE_URL = normalizeBaseUrl(process.env.STMARY_MOODLE_BASE_URL || "http://34.30.231.58");

const UNIT_TITLES = {
  1: "Exploring & Preparing for the World of Work",
  2: "Planning and Financial Management to Help Meet Postsecondary Goals",
  3: "Developing the Skills, Strategies, and Habits Needed to Succeed",
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-glc2o-stmary-localizer/1.0");
...
async function main() {
  ...
  const teacherResources = uniqueByPath([
    ...
    ...item,
    role: item.moodleActivityId === "11680" ? "answer_keys" : item.role,
    teacherOnly: item.teacherOnly || /\bAnswer\b/i.test(item.label || "") || item.moodleActivityId === "11680",
  })));
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "GLC2O · Career Studies, Grade 10",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 78 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
