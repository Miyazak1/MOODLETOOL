...
function scrubSourceUrls(manifest) {
  ...
  if (/www\.esunnybrook\.com/i.test(manifest.sourceAudit?.coursePage || "")) {
    manifest.sourceAudit.coursePage = "Moodle course id 30";
    scrubbed++;
  }
  eachResource(manifest, (item) => {
    if (!item) return;
...
function writeSources(stats, folderPagesRewritten, removedThemeAttachments, youtubePages) {
  mkdirSync(dirname(sourcesPath), { recursive: true });
  const content = `# BBI1O Sources and Localization Notes

- Course source: authenticated SunnyBrook Moodle course shell, course id 30.
- Structure: legacy Moodle activity course organized by the visible Moodle sections: Introduction, Unit 1 Personal Finance, Unit 2 Business Fundamentals and Functions of Business, Unit 3 Entrepreneurship, Unit 4 International Business, Culminating Project, Final Exam, and Teacher Evaluation/Comments.
...
- Course documents and planning: BBI1O/BBI2O course outline PDF and four current Moodle Unit Plan DOCX files were localized.
- Lesson materials and assessments: Moodle assignment/forum pages plus PDFs, DOCX/DOC worksheets, XLSX templates, PPT decks, JPG rubrics/templates, learning logs, culminating project, final exam, and teacher-evaluation pages were localized from Moodle.
- Reading materials: the Unit 1 Reading Materials folder downloaded one budget PDF and was rewritten as a local file-list page.
- Video: no downloadable Moodle MP4 files were exposed. ${youtubePages} Moodle activity page(s) include embedded YouTube references; those remain external embeds because Moodle did not expose downloadable source files for them.
- iSpring/H5P: no iSpring or H5P packages were visible in the current Moodle shell.
- Unavailable resources: none; every visible Moodle activity localized successfully.
...
function ensureSourceNotes(manifest) {
  ...
  manifest.courseDownloads.push({
    label: "BBI1O Sources and Localization Notes",
    type: "md",
...
function updateCatalog(stats) {
  ...
  if (entry) {
    entry.title = "Introduction to Business, Grade 9, Open";
    entry.level = "Grade 9";
    entry.status = "ready";
    entry.manifestUrl = "/courseware/BBI1O/course-manifest.json";
    entry.baseUrl = "/courseware/BBI1O/";
...
function updateRoadmap(stats) {
  ...
  if (entry) {
    ...
    entry.moodle = {
      coursePage: "Moodle course id 30",
      outlineStatus: "ready",
      outlineUrl: "",
      bookCount: 0,
    ...
    entry.localEvidence = {
      ...
      lessonPlans: 0,
      ispringFiles: 0,
      outlineExamples: ["BBI1O/BBI2O Course Outline PDF", "BBI1O Unit Plan DOCX folder files"],
    };
    entry.nextActions = [];
  }
...
writeSources(stats, folderPagesRewritten, removedThemeAttachments, youtubePages);
...
manifest.sourceAudit = {
  ...
  unitPlanStatus: "current Moodle unit-plan folder localized with four DOCX files",
...
updateCatalog(stats);
updateRoadmap(stats);
console.log(`BBI1O finalized: units ${stats.units}; lessons ${stats.lessons}; resources ${stats.resources}; attachments ${stats.attachments}; folders ${folderPagesRewritten}.`);
