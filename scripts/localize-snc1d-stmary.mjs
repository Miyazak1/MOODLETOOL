...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "snc1d-stmary-sections");
const BASE_URL = normalizeBaseUrl(process.env.STMARY_MOODLE_BASE_URL || "http://34.30.231.58");

const UNIT_TITLES = {
  1: "Biology: Sustainable Ecosystems",
  2: "Chemistry: Atoms, Elements, and Compounds",
  3: "Earth and Space Science: The Study of the Universe",
...
function curriculumGuideResources() {
  const rel = "course-resources/curriculum-guides/ontario-curriculum-grades-9-and-10-science-2008-revised.pdf";
  const abs = join(COURSE_ROOT, rel);
  if (!existsSync(abs)) return [];
  return [{
    label: "Ontario Curriculum, Grades 9 and 10: Science, 2008 (Revised)",
    type: "pdf",
    ...
    textPreview: "Official Ontario Ministry curriculum guidance for Grades 9 and 10 Science, including Science, Grade 9, Academic (SNC1D).",
  }];
}
...
async function main() {
  ...
  const courseDownloads = [
    activityById.get("11084"),
    activityById.get("11085"),
    activityById.get("11449"),
    activityById.get("11452"),
    ...curriculumGuideResources(),
  ].filter(Boolean);
  const teacherResources = uniqueByPath([
    teacherPacket,
    activityById.get("11454"),
  ...
  ].filter(Boolean).map((item) => ({ ...item, teacherOnly: item.teacherOnly || /\bAnswer\b/i.test(item.label || "") || item.moodleActivityId === "11454" })));
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "SNC1D · Science, Grade 9",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 77 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
    ...
    texts: textResources(),
    units,
    courseSections: [courseOverview, finalSection, teacherPacket],
    teacherResources,
