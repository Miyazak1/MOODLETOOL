...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "olc4o-stmary-sections");
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-olc4o-stmary-localizer/1.0");
...
function filteredLinksForSection(sectionNo) {
  ...
  if (sectionNo === 5) return links.filter((link) => /Culminating/i.test(link.text || ""));
  if (sectionNo === 7) return [];
...
async function main() {
  ...
  const culminatingSection = await buildCourseSection(5, "Culminating", "course-sections/culminating", "culminating");
  ...
  const courseDownloads = [
    activityById.get("10514"),
    activityById.get("10515"),
    activityById.get("10585"),
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "OLC4O · Ontario Secondary School Literacy Course, Grade 12",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 69 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
      ...
      h5pActivityExitCardsExcluded: sectionJson(7).modLinks.filter((link) => /h5pactivity|Exit Card/i.test(`${link.href} ${link.text}`)).length,
      downloadFailures,
