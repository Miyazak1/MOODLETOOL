...
function extractIntro(html) {
  ...
  if (match?.[1]) return match[1];
  const looseIntro = /<div\b[^>]*\bclass=["'][^"']*\bactivity-description\b[^"']*["'][^>]*\bid=["']intro["'][^>]*>([\s\S]*?)<\/div>\s*<\/div>/i.exec(html)?.[1];
  if (looseIntro) return looseIntro;

  const main = /<div\b[^>]*\brole=["']main["'][^>]*>([\s\S]*?)<\/div>\s*<div\b[^>]*\bclass=["'][^"']*\bcol-md-4\b/i.exec(html)?.[1]
    || /<div\b[^>]*\brole=["']main["'][^>]*>([\s\S]*?)<\/section>/i.exec(html)?.[1]
    || "";
  const noOverflow = /<div\b[^>]*\bclass=["'][^"']*\bno-overflow\b[^"']*["'][^>]*>([\s\S]*?)<\/div>/i.exec(main)?.[1];
  return noOverflow || main;
}
...
manifest.course.title ||= "BBB4M · International Business Fundamentals";
manifest.course.source ||= "SunnyBrook Moodle offline courseware";
manifest.navigation ||= { primary: "unit", secondary: "lesson" };

manifest.courseDownloads = manifest.courseDownloads.filter((item) => item.role !== "course_outline");
manifest.teacherResources = [];
...
const finalSection = await buildCourseSectionPage({
  sectionNumber: 6,
  title: "Culminating and Final Exam",
...
const courseActivities = [
  { id: 7241, title: "BBB4M Course Outline", role: "course_outline", teacherUse: "course_setup" },
  { id: 7242, title: "Learning Log", role: "learning_log", teacherUse: "student_tracking_template" },
  { id: 7336, title: "Culminating Assignment Dropbox", role: "culminating_submission", teacherUse: "assessment_preparation" },
  { id: 9383, title: "Final Exam Dropbox", role: "final_exam_submission", teacherUse: "assessment_preparation" },
];
for (const activity of courseActivities) {
  ...
  if (["culminating_submission", "final_exam_submission"].includes(record.role)) {
...
const unitActivities = {
  1: {
    evaluations: [
      ["Unit 1 - Assignment 1(AOL)", "assign", 7246],
      ["Unit 1 Test", "quiz", 7247],
    ],
    reflectionAndLogs: [
      ["Unit 1 - KWL Dropbox", "assign", 7261, "kwl_dropbox"],
      ["Unit 1 - Reflection Summary Dropbox", "assign", 7262, "reflection_dropbox"],
      ["Unit 1- Learning Log", "assign", 7263, "unit_learning_log"],
    ],
    lessonDropboxes: [7249, 7251, 7253, 7254, 7256, 7258, 7260].map((id, index) => [`Unit 1 - Lesson ${index + 1}`, "assign", id]),
    answerPages: [
      ["Unit 1 - Lesson 1 (Answer)", "page", 7250],
      ["Unit 1 - Lesson 2 (Answer)", "page", 7252],
      ["Unit 1 - Lesson 4 (Answer)", "page", 7255],
      ["Unit 1 - Lesson 5 (Answer)", "page", 7257],
      ["Unit 1 - Lesson 6 (Answer)", "page", 7259],
    ],
  },
  2: {
    evaluations: [
      ["Unit 2- Assignment 1 (AOL)", "assign", 7267],
      ["Unit 2 Test", "quiz", 9384],
    ],
    reflectionAndLogs: [
      ["Unit 2 - KWL Dropbox", "assign", 7285, "kwl_dropbox"],
      ["Unit 2 - Reflection Summary Dropbox", "assign", 7286, "reflection_dropbox"],
      ["Unit 2- Learning Log", "assign", 7287, "unit_learning_log"],
    ],
    lessonDropboxes: [7270, 7272, 7274, 7275, 7277, 7279, 7281, 7283].map((id, index) => [`Unit 2 - Lesson ${index + 1}`, "assign", id]),
    answerPages: [7271, 7273, 7276, 7278, 7280, 7282, 7284].map((id, index) => [`Unit 2 - Lesson ${index + 1} Answer`, "page", id]),
  },
  3: {
    evaluations: [
      ["Unit 3 - Assignment 1 (AOL)", "assign", 7291],
      ["Unit 3 Test", "quiz", 9385],
    ],
    reflectionAndLogs: [
      ["Unit 3 - KWL Dropbox", "assign", 7310, "kwl_dropbox"],
      ["Unit 3 - Reflection Summary Dropbox", "assign", 7311, "reflection_dropbox"],
      ["Unit 3- Learning Log", "assign", 7312, "unit_learning_log"],
    ],
    lessonDropboxes: [7294, 7296, 7298, 7300, 7302, 7304, 7306, 7308].map((id, index) => [`Unit 3 - Lesson ${index + 1}`, "assign", id]),
    answerPages: [7295, 7297, 7299, 7301, 7303, 7305, 7307, 7309].map((id, index) => [`Unit 3 - Lesson ${index + 1} Answer`, "page", id]),
  },
  4: {
    evaluations: [
      ["Unit 4 - Assignment 1 (AOL)", "assign", 7316],
      ["Unit 4 Test", "quiz", 9386],
    ],
    reflectionAndLogs: [
      ["Unit 4 - KWL Dropbox", "assign", 7333, "kwl_dropbox"],
      ["Unit 4 - Reflection Summary Dropbox", "assign", 7334, "reflection_dropbox"],
      ["Unit 4- Learning Log", "assign", 7335, "unit_learning_log"],
    ],
    lessonDropboxes: [7319, 7321, 7323, 7325, 7327, 7329, 7331].map((id, index) => [`Unit 4 - Lesson ${index + 1}`, "assign", id]),
    answerPages: [7320, 7322, 7324, 7326, 7328, 7330, 7332].map((id, index) => [`Unit 4 - Lesson ${index + 1} Answer`, "page", id]),
  },
};
...
manifest.sourceAudit.coursePage = `https://www.esunnybrook.com/course/view.php?id=${moodleCourseId}`;
...
manifest.sourceAudit.courseSectionLocalization = {
  ...
  teacherPacket: "not present in the visible BBB4M Moodle shell; no empty Teacher Packet card was created",
...
manifest.sourceAudit.courseResourceExpectedActivities = {
  courseOutline: 7241,
  learningLog: 7242,
  culminatingSubmission: 7336,
  finalExamSubmission: 9383,
};
...
manifest.sourceAudit.teacherPacket = {
  status: "not_exposed_in_moodle_shell",
  evidence: "Moodle course id 70 shows Course Overview, Unit 1-4, Culminating and Final Exam, and Exit Cards; no Teacher Packet section was visible in the inspected course structure.",
};
...
if (existsSync(catalogPath)) {
  ...
  if (row) {
    row.status = "ready";
    row.notes = "Moodle book lessons, localized iSpring, Moodle course/final/unit activities, documents/PDFs, official curriculum, and source audit are prepared; iSpring/video playback resources are non-downloadable.";
...
console.log(JSON.stringify({
  ...
  unitResources: manifest.units.map((unit) => ({ unit: unit.unit, resources: Object.fromEntries(Object.entries(unit.unitResources || {}).map(([key, value]) => [key, value.length])) })),
}, null, 2));
