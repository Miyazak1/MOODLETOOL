...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "fsf1d-stmary-sections");
...
const COURSE_ACTIVITY_ROLES = new Map([
  ["11692", { role: "course_outline", teacherUse: "course_planning" }],
  ["11693", { role: "learning_log", teacherUse: "student_tracking_template" }],
  ["11990", { role: "final_exam_submission", teacherUse: "assessment_preparation" }],
  ["11992", { role: "final_project_submission", teacherUse: "assessment_preparation" }],
  ["11997", { role: "answer_keys", teacherUse: "teacher_reference", teacherOnly: true }],
]);
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-fsf1d-stmary-localizer/1.0");
...
async function localizeActivity(link) {
  ...
  if (mod === "h5pactivity") {
    ...
    let packageUrl = /(?:href|src)=["']([^"']*\/pluginfile\.php\/[^"']*\/mod_h5pactivity\/package\/[^"']*\.h5p[^"']*)["']/i.exec(html)?.[1] || "";
    if (!packageUrl) {
      const embedUrl = /(?:href|src)=["']([^"']*\/h5p\/embed\.php\?[^"']+)["']/i.exec(html)?.[1] || "";
      if (embedUrl) packageUrl = new URL(decodeEntities(embedUrl), link.href).searchParams.get("url") || "";
    }
    if (packageUrl) {
      const localized = await downloadResource(new URL(decodeEntities(packageUrl), link.href).toString(), toPosix(join(relDir, "files")), title);
      const item = {
        ...localized,
        label: title,
        type: "h5p",
...
function filteredLinksForSection(sectionNo) {
  ...
  if (sectionNo === 6) return links.filter((link) => /Final Project|Final Exam|Exam Submission|Project Submission/i.test(link.text || ""));
...
function writeTextbookReferenceFile(hasTextbook) {
  const rel = "texts/fsf1d-textbook-reference/index.html";
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>FSF1D Textbook Reference</title>
  <style>
...
<body>
  <main>
    <h1>FSF1D Textbook Reference</h1>
    <p class="notice">${hasTextbook ? "A legally obtained local textbook copy has been provided by the user and included in this FSF1D course package." : "No local textbook copy is included. No matching local FSF1D/Core French textbook file was found during this import."}</p>
    <h2>Textbook Status</h2>
    <dl>
      <dt>Primary title</dt>
      <dd>Not identified from the localized Moodle source during import</dd>
      <dt>Publisher</dt>
      <dd>Not identified</dd>
      <dt>Course match</dt>
      <dd>FSF1D Core French, Grade 9, Academic</dd>
      <dt>Moodle evidence</dt>
      <dd>FSF1D St. Mary Moodle course id 80; Course Outline activity is preserved separately</dd>
    </dl>
    <h2>Evidence</h2>
    <ul>
      <li>The local <code>D:/工作文件/SUNNYBROOK/docs</code> folder did not contain a clearly matching FSF1D/Core French textbook PDF.</li>
      <li>No unrelated science or biology textbook was copied into this course.</li>
      <li>The source course used for this import is St. Mary Moodle course id 80.</li>
    </ul>
  </main>
</body>
</html>
`;
  ...
  return {
    label: "FSF1D Textbook Reference",
    type: "html",
    ...
    textPreview: "Records that no matching local FSF1D/Core French textbook file was identified.",
  };
}
...
function writeSourceAuditFile() {
  ...
  const abs = join(COURSE_ROOT, rel);
  ensureDir(dirname(abs));
  const body = `# FSF1D Sources

- Source Moodle course: ${BASE_URL}/course/view.php?id=${COURSE_ID}
- Moodle sections exported to: ossd-course-portal/inbox/fsf1d-stmary-sections
- Moodle book raw exports:
  - Unit 1: inbox/moodle-book-raw-FSF1D-U01.json
  - Unit 2: inbox/moodle-book-raw-FSF1D-U02.json
  - Unit 3: inbox/moodle-book-raw-FSF1D-U03.json
  - Unit 4: inbox/moodle-book-raw-FSF1D-U04.json
- Moodle section 0 course introduction is preserved as a course-level HTML page
  with course description and prerequisite text.
- Moodle section 1 Course Overview is preserved with Course Outline and Learning
  Log resources.
- Moodle sections 2-5 are Units 1-4 with Moodle Book lessons.
- Moodle section 6 is Unit 5: Final Exam and Final Project, preserved as
  course-level final evaluation context.
- Moodle section 7 Teacher Packet contains Answer Keys and is teacher-only.

Textbook: not identified.

No matching local FSF1D/Core French textbook file was found under
D:/工作文件/SUNNYBROOK/docs during this import. No unrelated textbook was copied.
`;
  writeFileSync(abs, body, "utf8");
  return {
    id: "fsf1d-source-audit",
    title: "FSF1D Text And Source Audit",
    type: "source_audit",
    units: [1, 2, 3, 4],
    copyrightStatus: "local_audit_note",
    sourceStatus: "created_from_local_source_review",
    notes: "Records Moodle source exports, section 0/1 preservation, final evaluation resources, H5P exit cards, and textbook audit.",
    materials: [{
      label: "FSF1D Text And Source Audit",
      type: "md",
...
async function main() {
  ...
  const finalSection = await buildCourseSection(6, "Unit 5: Final Exam and Final Project", "course-sections/unit-5-final-exam-final-project", "final_exam_final_project", { optional: true });
  const teacherPacket = await buildCourseSection(7, "Teacher Packet", "course-sections/teacher-packet", "teacher_packet", { optional: true });
  ...
  const finalExam = activityById.get("11990");
  const finalProject = activityById.get("11992");

  const courseDownloads = [
    activityById.get("11692"),
    activityById.get("11693"),
    finalExam,
    finalProject,
  ...
  const teacherResources = uniqueByPath([
    teacherPacket,
    activityById.get("11997"),
    ...localizedActivities.filter((item) => ANSWER_RE.test(item.label || "")),
    ...evaluations,
  ].filter(Boolean).map((item) => ({ ...item, teacherOnly: item.teacherOnly || ANSWER_RE.test(item.label || "") || item.moodleActivityId === "11997" })));
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "FSF1D - Core French, Grade 9, Academic",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 80",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
      ...
      finalSectionPresent: Boolean(finalSection || finalExam || finalProject),
      teacherPacketPresent: Boolean(teacherPacket || activityById.get("11997")),
      exitCardsSectionPresent: localizedActivities.some((item) => item.category === "moodle_h5pactivity"),
      ...
      textbookReference: {
        ...
        evidence: "No matching local FSF1D/Core French textbook file was found during import; Course Outline activity is preserved for teacher review.",
      ...
      textbookAudit: {
        status: "not_identified",
        textbook: "Not identified",
        policyDocument: "Not included; no local FSF1D/French curriculum PDF was identified in the workspace.",
        evidence: "Searched D:/工作文件/SUNNYBROOK/docs for obvious FSF/French/Core French textbook and curriculum files; none matched.",
        decision: "Do not add unrelated science/biology textbooks or curriculum documents to FSF1D.",
        localPath: textbook ? textbook.path : undefined,
      },
    },
