...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "eng1d-stmary-sections");
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-eng1d-stmary-localizer/1.0");
...
function cleanHtmlFragment(htmlValue, attachmentMap = new Map(), context = {}) {
  ...
  body = body.replace(/<div[^>]*id=["']assign_files_tree[^"']*["'][^>]*>[\s\S]*?(?=<\/article>|<section\b|<h[1-6]\b|$)/gi, "");
  body = body.replace(/<div[^>]*class=["'][^"']*fileuploadsubmission(?:time)?[^"']*["'][^>]*>[\s\S]*?<\/div>/gi, "");
...
<head>
  ...
  <style>
    ...
    .content video { margin-left: auto; margin-right: auto; }
    .content .mediaplugin_videojs, .content .mediaplugin_videojs > div { margin-left: auto; margin-right: auto; max-width: 100%; }
...
async function main() {
  ...
  const courseDownloads = [
    activityById.get("11076"),
    activityById.get("11078"),
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "ENG1D · English, Grade 9, Academic",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 74 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
