...
function lessonPlanFor(unit, lesson, previousLesson) {
  ...
  const path = `plans/source/Unit ${unit} Lesson Plans/Unit ${unit} - Lesson ${lesson} Lesson Plan.docx`;
...
function unitPlanFor(unit, previousUnit) {
  ...
  const path = `plans/source/Unit Plans/SBI4U - Unit ${unit} - Unit Plan.docx`;
  if (!existsSync(join(courseRoot, path))) return null;
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 88,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=88",
