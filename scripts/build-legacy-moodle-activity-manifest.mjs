...
if (!configPath) {
  console.error("Usage: node scripts/build-legacy-moodle-activity-manifest.mjs --config path/to/config.json");
  process.exit(1);
}
...
const coursePage = `https://www.esunnybrook.com/course/view.php?id=${config.moodleCourseId}`;
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-legacy-manifest-builder/1.0");
...
function roleForLabel(label, mod = "") {
  for (const rule of config.roleRules || []) {
    if (new RegExp(rule.pattern, "i").test(label)) return rule.role;
  }
  if (/course outline/i.test(label)) return "course_outline";
  if (/lesson plans?|unit overview/i.test(label)) return "lesson_plan_reference";
  if (/success criteria|syllabus|attendance|learning skills|learning log|exit card|cheat sheet|note taking/i.test(label)) return "course_document";
  if (/test|exam/i.test(label)) return "assessment";
  if (/discussion|forum/i.test(label) || mod === "forum") return "discussion";
  if (/activity|assignment|submission|dropbox|worksheet|journal|sheet|poster|isp|guide book/i.test(label)) return "assignment";
  if (/lesson|grammar|slides|writing|story|fiction|article|media|canada|charter|dictionary|vocab|resource/i.test(label)) return "lesson_material";
...
const sectionSpecs = new Map(config.sections.map((section) => [String(section.id), section]));
...
const sectionActivities = new Map(config.sections.filter((section) => section.unit > 0).map((section) => [section.unit, []]));
...
const units = config.sections.filter((section) => section.unit > 0).map((section) => {
  const activities = sectionActivities.get(section.unit) || [];
  const lessons = activities.map((item, index) => lessonRecord(section.unit, index + 1, item.label, item));
...
console.log(`${course} manifest rebuilt: units ${units.length}; lessons ${manifest.sourceAudit.lessonCount}; activities ${totalActivities}.`);
