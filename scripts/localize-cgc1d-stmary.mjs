...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "cgc1d-stmary-sections");
const BASE_URL = normalizeBaseUrl(process.env.STMARY_MOODLE_BASE_URL || "http://34.30.231.58");

const UNIT_TITLES = {
  1: "Canadian Connections and Methods of Geographical Inquiry",
  2: "Canada's Ecozones and Cultural Connections",
  3: "Canada's Economic Connections",
  4: "Canada's Global and Future Connections",
};
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-cgc1d-stmary-localizer/1.0");
...
function filteredLinksForSection(sectionNo) {
  ...
  if (sectionNo === 6) return links.filter((link) => /Culminating/i.test(link.text || ""));
...
async function main() {
  ...
  const courseDownloads = [
    courseOverview,
    activityById.get("11758"),
    activityById.get("11762"),
    finalSection,
    activityById.get("12197"),
    activityById.get("12198"),
    teacherPacket,
  ].filter(hasMeaningfulResource);
  const teacherResources = uniqueByPath([
    teacherPacket,
    activityById.get("12199"),
  ...
  ].filter(hasMeaningfulResource).map((item) => ({ ...item, teacherOnly: item.teacherOnly || /\bAnswer\b/i.test(item.label || "") || item.moodleActivityId === "12199" })));
  const courseSections = [courseOverview, finalSection, teacherPacket].filter(hasMeaningfulResource);
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "CGC1D · Issues in Canadian Geography, Grade 9, Academic",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 82 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
