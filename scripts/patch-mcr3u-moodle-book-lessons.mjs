...
const unitTitles = {
  1: "Functions, Quadratics, Polynomials and Rational Expressions",
...
function lessonPlanFor(unit, lesson) {
  const path = `plans/source/Unit ${unit}/Lesson ${lesson}.docx`;
...
function unitPlanFor(unit) {
  const path = `plans/source/Unit ${unit}/Unit ${unit} Plan.docx`;
  if (!existsSync(join(courseRoot, path))) return null;
  return fileRecord(`Unit Plan - ${unitTitles[unit] || `Unit ${unit}`}`, path, "unit_plan", "unit_plan");
}

function withoutPlayableDownload(item) {
  if (!item) return item;
  const { downloadPath, downloadUrl, downloadBytes, ...playableOnly } = item;
  return playableOnly;
}
...
manifest.units = rawByUnit.map(({ unit, raw }) => {
  ...
  const lessons = raw.lessons.map((rawLesson, index) => {
    ...
    return {
      ...
      lessonPlan,
      ispring: (previousLesson.ispring || []).map(withoutPlayableDownload),
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 76,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=76",
