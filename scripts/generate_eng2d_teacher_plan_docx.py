...
COURSE_ROOT = WORKSPACE_ROOT / "courseware" / "ENG2D"
MANIFEST_PATH = COURSE_ROOT / "course-manifest.json"
REPORT_PATH = REPO_ROOT / "deployment" / "ENG2D-docx-plans-report.json"
...
UNITS = {
    1: {
        "strand": "Reading and Literature Studies / Oral Communication / Writing",
        "focus": "Macbeth, Shakespearean drama, historical and contextual background, act-by-act plot development, character motivation, quotation evidence, theme, conflict, and dramatic analysis.",
        "essential": [
            "How do Shakespeare's language, context, and dramatic choices shape meaning in Macbeth?",
            "How do ambition, power, guilt, and conflict influence character decisions and consequences?",
            "How can students use precise quotation evidence to explain character, theme, and dramatic effect?",
        ],
    },
    2: {
        "strand": "Media Studies / Writing / Oral Communication",
        "focus": "media literacy, advertising techniques, persuasive messaging, audience and purpose, advertisement analysis, and responsible online communication/netiquette.",
        "essential": [
            "How do media creators use techniques to influence audience interpretation and behaviour?",
            "How can students identify purpose, audience, bias, and implied messages in advertising?",
            "What does responsible, effective communication look like in online spaces?",
        ],
    },
    3: {
        "strand": "Writing / Language Conventions / Reading and Literature Studies",
        "focus": "grammar, quotation integration, proofreading, descriptive and expository writing, Shakespearean reading strategies, paragraph and essay structure, and literary devices.",
        "essential": [
            "How do writers use conventions, structure, and revision to make meaning clear?",
            "How can students integrate quotations and textual evidence accurately and effectively?",
            "How do literary devices and writing choices shape a reader's understanding?",
        ],
    },
    4: {
        "strand": "Writing / Media and Communication / Reading and Literature Studies",
        "focus": "sentence structures, purpose, audience, tone, diction, punctuation, literary devices, writing features, and final writing refinement.",
        "essential": [
            "How do sentence structure, diction, punctuation, and tone affect clarity and voice?",
            "How can writers adapt language choices for purpose and audience?",
            "How can students use revision strategies to strengthen final written communication?",
        ],
    },
}
...
def unit_summary(unit, unit_info, lessons):
    ...
    return (
        f"This unit supports Grade 10 Academic English teachers in moving students through {unit['title']} with a clear cycle: "
...
def lesson_plan_doc(unit, lesson):
    ...
    set_text(rows[0].cells[2], "Subject: ENG2D English, Grade 10 Academic", bold=True)
    set_row_text(rows[1], f"Lesson Name: {lesson_title}")
    set_row_text(rows[2], f"Unit of Study: {unit['title']}")
    prior = (
        f"Connect this lesson to the previous reading, learning log, KWL reflection, or consolidation response in {unit['title']}. "
    ...
    set_row_text(
        ...
        + "\n".join(material_lines)
        + f"\nSource status: reconstructed_from_moodle_content. Generated from localized ENG2D Moodle resources on {GENERATED_AT}.",
    )
...
def unit_plan_doc(unit):
    ...
    values = [
        ...
        (
            ...
            "\n".join(
                [
                    f"Students will read, interpret, discuss, and write about {unit['title']} using accurate English terminology and course-specific vocabulary.",
...
def file_record(label, relative_path, category, role):
    ...
    return {
        ...
        "teacherUse": "Teacher planning aid reconstructed from localized ENG2D course content; not an original Moodle teacher packet.",
...
def main():
        ...
        **(manifest.get("sourceAudit") or {}),
        "teacherPlanGeneration": {
            ...
            "basis": [
                ...
                "ENG2D unit sequence, lesson expectations, and instructional focus visible in localized Moodle course books",
    ...
    REPORT_PATH.write_text(
        json.dumps({"course": "ENG2D", "unitPlans": unit_count, "lessonPlans": lesson_count, "written": written, "generatedAt": GENERATED_AT}, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    print(json.dumps({"course": "ENG2D", "unitPlans": unit_count, "lessonPlans": lesson_count, "generatedAt": GENERATED_AT}, indent=2))
