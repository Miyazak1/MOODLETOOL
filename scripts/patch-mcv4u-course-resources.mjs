...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-mcv4u-resource-localizer/1.0");
...
const courseActivities = [
  { id: 8108, title: "MCV4U Course Outline", role: "course_outline", teacherUse: "course_setup" },
  { id: 8109, title: "Learning Log", role: "learning_log", teacherUse: "student_tracking_template" },
  { id: 8192, title: "Final Exam Dropbox", role: "final_exam_submission", teacherUse: "assessment_preparation" },
  { id: 8193, title: "Culminating Dropbox", role: "culminating_submission", teacherUse: "assessment_preparation" },
];
...
const legacyOutline = manifest.courseDownloads.find((item) => item.path === "plans/course/MCV4U_Course_Outline.docx");
if (legacyOutline) {
  legacyOutline.label = "MCV4U Course Outline.docx";
  legacyOutline.role = "course_outline";
  legacyOutline.category = "course_document";
  delete legacyOutline.moodleActivityId;
  delete legacyOutline.url;
}

const unitActivities = {
  1: {
    evaluations: [
      ["Unit 1 - Quiz (AOL)", "quiz", 8113],
      ["Unit 1 Assignment (AOL)", "assign", 8114],
      ["Unit 1 - Test (AOL)", "quiz", 8115],
    ],
    reflectionAndLogs: [
      ["Unit 1 - Reflection Summary Dropbox", "assign", 8131, "reflection_dropbox"],
      ["Unit 1 - KWL Dropbox", "assign", 8132, "kwl_dropbox"],
    ],
    lessonDropboxes: [8117, 8119, 8121, 8123, 8125, 8127, 8129].map((id, index) => [`Unit 1 - Lesson ${index + 1}`, "assign", id]),
    answerPages: [8118, 8120, 8122, 8124, 8126, 8128, 8130].map((id, index) => [`Unit 1 - Lesson ${index + 1} Answer`, "page", id]),
  },
  2: {
    evaluations: [
      ["Unit 2 - Quiz (AOL)", "quiz", 8143],
      ["Unit 2 Test (AOL)", "quiz", 8144],
      ["Unit 2 - Assignment (AOL)", "assign", 8145],
    ],
    reflectionAndLogs: [
      ["Unit 2 - KWL Dropbox", "assign", 8156, "kwl_dropbox"],
      ["Unit 2 - Reflection Summary Dropbox", "assign", 8157, "reflection_dropbox"],
    ],
    lessonDropboxes: [8147, 8149, 8151, 8153, 8155].map((id, index) => [`Unit 2 - Lesson ${index + 1}`, "assign", id]),
    answerPages: [8148, 8150, 8152, 8154].map((id, index) => [`Unit 2 - Lesson ${index + 1} Answer`, "page", id]),
  },
  3: {
    evaluations: [
      ["Unit 3 - Quiz (AOL)", "quiz", 8167],
      ["Unit 3 - Test (AOL)", "quiz", 8168],
      ["Unit 3 Assignment (AOL)", "assign", 8169],
    ],
    reflectionAndLogs: [
      ["Unit 3 - KWL Dropbox", "assign", 8183, "kwl_dropbox"],
      ["Unit 3 - Reflection Summary Dropbox", "assign", 8184, "reflection_dropbox"],
    ],
    lessonDropboxes: [8171, 8173, 8175, 8177, 8179, 8181].map((id, index) => [`Unit 3 - Lesson ${index + 1}`, "assign", id]),
    answerPages: [8172, 8174, 8176, 8178, 8180, 8182].map((id, index) => [`Unit 3 - Lesson ${index + 1} Answer`, "page", id]),
  },
};
...
manifest.sourceAudit.courseSectionLocalization = {
  ...
  sections: [
    ...
    { sectionNumber: 5, role: "final_examination_culminating", path: finalSection.path, attachments: finalSection.attachments.length },
  ],
...
manifest.sourceAudit.courseResourceExpectedActivities = {
  courseOutline: 8108,
  learningLog: 8109,
  finalExamSubmission: 8192,
  culminatingSubmission: 8193,
};
...
manifest.sourceAudit.moodleUnitTitleAlignment = {
  ...
  note: "Display unit titles follow the authenticated Moodle shell. Local planning files use a different four-folder numbering scheme, so their unit plans remain related planning files instead of authoritative Moodle unit titles.",
};
