...
const SECTION_DIR = resolve(REPO_ROOT, "inbox", "avi4m-stmary-sections");
...
async function request(url, options = {}, redirects = 0) {
  ...
  headers.set("user-agent", "ossd-course-portal-avi4m-stmary-localizer/1.0");
...
async function main() {
  ...
  const courseDownloads = [
    activityById.get("10626"),
    activityById.get("10627"),
  ...
  const manifest = {
    ...
    course: {
      code: COURSE,
      title: "AVI4M · Visual Arts, Grade 12, University",
      audience: "Teachers preparing OSSD lessons",
      source: "St. Mary Moodle course id 70 and existing legal local source files",
    },
    sourceAudit: {
      moodleHost: BASE_URL,
