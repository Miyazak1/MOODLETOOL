...
const unitTitles = {
  1: "Business, Trade, and the Canadian Economy",
...
function lessonPlanFor(unit, lesson, previousLesson) {
  ...
  const path = `plans/source/U${unit}/Unit ${unit} - Lesson ${lesson} Lesson Plan.docx`;
...
function unitPlanFor(unit, previousUnit) {
  ...
  const path = `plans/source/BBB4M - Unit ${unit} - Unit Plan.docx`;
  if (!existsSync(join(courseRoot, path))) return null;
...
manifest.course = {
  ...(manifest.course || {}),
  code: course,
  title: "BBB4M · International Business Fundamentals",
...
manifest.sourceAudit = {
  ...(manifest.sourceAudit || {}),
  moodleCourseId: 70,
  coursePage: "https://www.esunnybrook.com/course/view.php?id=70",
