...
COURSE_ROOT = WORKSPACE_ROOT / "courseware" / "SBI4U"
MANIFEST_PATH = COURSE_ROOT / "course-manifest.json"
REPORT_PATH = REPO_ROOT / "deployment" / "SBI4U-docx-plans-report.json"
...
STRANDS = {
    1: {
        "code": "B",
        "title": "Biochemistry",
        "focus": "chemical bonding, water, functional groups, biological macromolecules, enzymes, cell organelles, membrane structure, and diffusion.",
        "essential": [
            "How do chemical structures and interactions explain the functions of biological molecules?",
            "How do enzymes, organelles, membranes, and diffusion support cellular processes?",
            "How can biochemical models and investigations be used to explain living systems?",
        ],
    },
    2: {
        "code": "C",
        "title": "Metabolic Processes",
        "focus": "glycolysis, cellular respiration, Krebs cycle, electron transport, photosynthesis, light-dependent reactions, and light-independent reactions.",
        "essential": [
            "How do cells transform matter and energy through respiration and photosynthesis?",
            "How do metabolic pathways depend on enzymes, membranes, gradients, and electron carriers?",
            "How can models and evidence explain the relationship between metabolism and ecosystem energy flow?",
        ],
    },
    3: {
        "code": "D",
        "title": "Molecular Genetics",
        "focus": "DNA structure and replication, protein synthesis, gene regulation, DNA sequencing, mutations, and applications of molecular genetics.",
        "essential": [
            "How does DNA store, copy, regulate, and express genetic information?",
            "How do mutations and gene regulation affect proteins, traits, and biological function?",
            "How should genetic technologies be evaluated using scientific evidence and ethical reasoning?",
        ],
    },
    4: {
        "code": "E",
        "title": "Homeostasis",
        "focus": "homeostasis, nervous system structure and processes, hormones, endocrine regulation, stress and blood sugar regulation, and reproductive regulation.",
        "essential": [
            "How do feedback systems maintain internal balance in animals?",
            "How do the nervous and endocrine systems coordinate responses to internal and external change?",
            "How can homeostatic disorders and technologies be understood through structure-function relationships?",
        ],
    },
    5: {
        "code": "F",
        "title": "Population Dynamics",
        "focus": "characteristics of populations, population growth and regulation, interactions between individuals, and human population growth.",
        "essential": [
            "How do population characteristics and interactions affect population change over time?",
            "How can models, data, and limiting factors explain carrying capacity and growth patterns?",
            "How do human population trends connect biology to ecological, social, and ethical decisions?",
        ],
    },
}
...
def expectation_codes(text):
    return sorted(set(re.findall(r"\b[A-F][1-3](?:\.\d+)?\b", text or "")))[:18]
...
def split_sentences(text, limit):
    ...
    if not text:
        return []
    parts = re.split(r"(?<=[.;:])\s+|(?=\b[A-F][1-3](?:\.\d+)?\b)", text)
...
def learning_goals(expectations_text, lesson_title):
    ...
    if match:
        ...
        if goals:
            return goals[:4]
    return [
        f"Explain the central biology concept in {lesson_title} using accurate SBI4U vocabulary.",
...
def lesson_success_criteria(lesson_title, codes):
    ...
    return [
        f"I can describe the main idea in {lesson_title} using correct biology vocabulary.",
...
def unit_summary(unit, strand, lessons):
    ...
    return (
        f"This unit develops the {strand['title']} strand through {len(lessons)} linked SBI4U lessons. "
...
def lesson_plan_doc(unit, lesson):
    ...
    set_text(rows[0].cells[2], "Subject: SBI4U Biology, Grade 12 University Preparation", bold=True)
    set_row_text(rows[1], f"Lesson Name: {lesson['title']}")
    set_row_text(rows[2], f"Unit of Study: {unit['title']}")
    prior = (
        f"Students should activate vocabulary and process knowledge from previous SBI4U work in {unit['title']}. "
    ...
    set_row_text(
        ...
        + f"\nSource status: reconstructed_from_moodle_content. Generated from localized SBI4U Moodle resources on {GENERATED_AT}.",
    )
...
def unit_plan_doc(unit):
    ...
    values = [
        ...
        (10, "Year Level"),
        (11, "Grade 12, University Preparation Biology"),
        ...
        (
            ...
            "\n".join(
                [
                    f"Students will explain and apply major concepts in {unit['title']} using accurate SBI4U biology vocabulary.",
...
def file_record(label, relative_path, category, role):
    ...
    return {
        ...
        "teacherUse": "Teacher planning aid reconstructed from localized SBI4U course content; not an original Moodle teacher packet.",
...
def main():
        ...
        **(manifest.get("sourceAudit") or {}),
        "teacherPlanGeneration": {
            ...
            "basis": [
                ...
                "SBI4U strand and expectation evidence already present in the localized course pages",
                "Ontario Science Grades 11 and 12 curriculum framework, including Strand A scientific investigation skills and career exploration as an integrated planning lens",
    ...
    REPORT_PATH.write_text(
        json.dumps({"course": "SBI4U", "unitPlans": unit_count, "lessonPlans": lesson_count, "written": written, "generatedAt": GENERATED_AT}, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    print(json.dumps({"course": "SBI4U", "unitPlans": unit_count, "lessonPlans": lesson_count, "generatedAt": GENERATED_AT}, indent=2))
