...
本项目已提供 `sync:oss`，它不会直接让 Portal 去扫描 OSS，而是把本地课程文件同步计划和发布结果写入 `deployment/asset-registry.json`。Portal 的 `hybrid` 模式读取这个登记表：命中的资源走 CDN，未命中的资源继续走本机 `/embed/t/...`，所以可以按课程灰度迁移。

```bash
npm run sync:oss -- --course HFC3M --bucket oss://moodletool-courseware --cdn-base-url https://cdn.moodletool.work/courseware-active --dry-run
npm run sync:oss -- --course HFC3M --bucket oss://moodletool-courseware --cdn-base-url https://cdn.moodletool.work/courseware-active --apply
...
当前 Node 脚本负责生成计划、调用 `ossutil`、写入 `asset-registry.json` 和控制 metadata。后续如果需要更细的错误重试、断点续传状态或批量删除审批，可以再切到阿里云 OSS SDK。

### 8.3 一键编排流程

日常建议优先使用 `pipeline:media-delivery`，它会按顺序执行视频审计、压缩计划、OSS 同步计划、CDN 预热清单和媒体交付门禁。默认是安全模式：不压缩真实视频、不上传 OSS。

HFC3M 灰度 dry-run：

```bash
npm run pipeline:media-delivery -- --course HFC3M --bucket oss://moodletool-courseware --cdn-base-url https://cdn.moodletool.work/courseware-active --asset-mode hybrid
```

确认 HFC3M 视频压缩后，只打开压缩 apply，OSS 仍保持 dry-run：

```bash
npm run pipeline:media-delivery -- --course HFC3M --apply-optimize --bucket oss://moodletool-courseware --cdn-base-url https://cdn.moodletool.work/courseware-active --asset-mode hybrid
```

确认 OSS/CDN 配置完成后，再打开 OSS apply 和强门禁：

```bash
npm run pipeline:media-delivery -- --course HFC3M --apply-oss --require-oss --bucket oss://moodletool-courseware --cdn-base-url https://cdn.moodletool.work/courseware-active --asset-mode hybrid
```

如果压缩和上传都已确认，也可以一次执行：

```bash
npm run pipeline:media-delivery -- --course HFC3M --apply-optimize --apply-oss --require-oss --bucket oss://moodletool-courseware --cdn-base-url https://cdn.moodletool.work/courseware-active --asset-mode hybrid
```

pipeline 会输出：

```text
deployment/media-delivery-pipeline-report.json
deployment/media-delivery-pipeline-report.md
```
...
```bash
npm run export:cdn-preheat -- --course HFC3M --cdn-base-url https://cdn.moodletool.work/courseware-active
```

输出：

```text
deployment/cdn-preheat-urls-HFC3M.txt
deployment/cdn-preheat-urls-HFC3M.json
deployment/cdn-preheat-urls-HFC3M.md
```

将 `.txt` 里的 URL 粘贴到阿里云 CDN 预热任务即可。预热完成后抽查：

```bash
npm run smoke:cdn-published -- --course HFC3M --cdn-base-url https://cdn.moodletool.work/courseware-active --limit 30
```

切换 Portal 到 `hybrid` 或 `cdn` 前，先运行媒体交付门禁：

```bash
npm run check:media-delivery -- --course HFC3M --bucket oss://moodletool-courseware --cdn-base-url https://cdn.moodletool.work/courseware-active --asset-mode hybrid --require-oss
```

如果仍处于本地准备阶段、不想把 OSS/CDN 缺项视为阻断，可以去掉 `--require-oss`。
...
- `OSS_BUCKET_URI`：同步工具使用的 OSS bucket，不是 Portal 播放时的公网出口。
- `COURSEWARE_ASSET_MODE`：`local`、`hybrid`、`cdn` 三种模式。
- `COURSEWARE_ASSET_BASE_URL`：发布资源的 CDN base URL。
- `COURSEWARE_ASSET_PREFIX`：OSS object key 前缀，默认 `courseware-active`。
- `COURSEWARE_ASSET_REGISTRY_FILE`：同步工具生成的发布登记表。`hybrid` 模式依赖它判断资源是否已发布。
...
```text
如果 COURSEWARE_ASSET_MODE=cdn:
  使用 CDN URL
如果 COURSEWARE_ASSET_MODE=hybrid 且 asset-registry 中存在该资源 object key:
  使用 CDN URL
否则:
...
npm run optimize:videos
npm run sync:oss
npm run pipeline:media-delivery
npm run export:cdn-preheat
npm run smoke:cdn-published
...
- `optimize:videos --apply` 和 `sync:oss --apply` 已接入跨进程课程锁，锁目录默认为 `deployment/locks/`。
- `pipeline:media-delivery` 已串联审计、压缩计划/执行、OSS 同步计划/执行、CDN 预热清单和 readiness 门禁，默认 dry-run，不会修改视频或上传 OSS。
- `asset-registry.json` 已调整为轻量发布登记表，默认只保留 Portal hybrid 模式需要的 OSS object key 列表，CDN URL 由 Portal 根据 `COURSEWARE_ASSET_BASE_URL` 生成。
- `export:cdn-preheat` 可从 `asset-registry.json` 导出 CDN 预热 URL 清单。
- `smoke:cdn-published` 可在 CDN 上线后抽查已发布资源，视频资源会用 Range 请求验证分段播放能力。
- `check:media-delivery` 可作为切换 `hybrid/cdn` 前的上线门禁，检查 ffmpeg/ffprobe、ossutil、OSS bucket、CDN base、审计报告、压缩计划、asset registry 和预热清单是否齐全。
- `check:production-env` 已校验 `COURSEWARE_ASSET_MODE`、`COURSEWARE_ASSET_BASE_URL`、`COURSEWARE_ASSET_REGISTRY_FILE` 和 `OSS_BUCKET_URI`，可提前发现 CDN/OSS 配置错误。
- Portal 已支持 `COURSEWARE_ASSET_MODE=local|hybrid|cdn`。
- `smoke:cdn-assets` 已验证：
  - `local` 模式继续使用 `/embed/t/...`。
  - `cdn` 模式下 iSpring `<base>` 和视频 `<source>` 使用 CDN URL。
  - `hybrid` 模式下登记表命中的资源使用 CDN，未命中的资源回退本地 token URL。
...
deployment/asset-registry.json
deployment/cdn-preheat-urls*.txt
deployment/cdn-preheat-urls*.json
deployment/cdn-preheat-urls*.md
deployment/media-delivery-pipeline-report.json
deployment/media-delivery-pipeline-report.md
```

其中 `oss-sync-plan.json` 可能达到几十 MB，属于部署生成物，已加入 `.gitignore`，不建议提交到 Git。`asset-registry.json` 也属于部署生成物，不提交到 Git，但会由 `sync:oss` 生成给 Portal hybrid 模式读取；当前 54 门课全量登记表约 6.9 MB。

服务器启用 CDN 灰度时建议：

如果需要重新生成生产 env，可以直接生成 hybrid 版本：

```bash
npm run generate:production-env -- --domain www.moodletool.work --courses HFC3M --asset-mode hybrid --oss-bucket oss://moodletool-courseware --cdn-base-url https://cdn.moodletool.work/courseware-active
```

如果是改已有 `.env.production`，至少加入或确认：
...
3. 对 HFC3M 执行 pipeline:media-delivery dry-run，确认审计、压缩计划、OSS 同步计划、预热清单和 readiness 输出。
4. 确认后执行 pipeline:media-delivery --apply-optimize，完成 HFC3M 高码率视频压缩。
5. 执行 pipeline:media-delivery --apply-oss --require-oss，完成 HFC3M 灰度上传、预热清单和上线门禁。
6. 在阿里云 CDN 控制台预热 deployment/cdn-preheat-urls-HFC3M.txt 中的 URL。
7. 执行 smoke:cdn-published --course HFC3M 抽查 CDN 已发布资源和视频 Range。
8. Portal 配置 COURSEWARE_ASSET_BASE_URL / COURSEWARE_ASSET_MODE=hybrid，并先让 HFC3M 走 CDN 资源。
```
