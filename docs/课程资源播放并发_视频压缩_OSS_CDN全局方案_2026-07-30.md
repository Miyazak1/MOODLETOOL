...
4. 执行 export:cdn-preheat --course HFC3M 并在阿里云 CDN 控制台预热这些 URL。
5. 执行 smoke:cdn-published --course HFC3M 抽查 CDN 已发布资源和视频 Range。
6. 执行 check:media-delivery --course HFC3M --require-oss，确认上线门禁通过。
7. Portal 配置 COURSEWARE_ASSET_BASE_URL / COURSEWARE_ASSET_MODE=hybrid，并先让 HFC3M 走 CDN 资源。
```
